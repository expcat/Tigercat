...
**下一步建议**：执行 PR-16（Table 单文件拆分 ~10 子文件，规模大）或 PR-19a..k（视觉现代化，可按组小步推进）。PR-17（CodeEditor/RichTextEditor engine 化）规模大，建议放在 PR-16 之后单独排期；DataTableWithToolbar 状态机抽离仍随 PR-16 一并评估。
