...
**下一步建议**：执行 PR-16（Table 单文件拆分 ~10 子文件）或 PR-2（Tailwind v3→v4 内部，需中等回归验证）。PR-17（CodeEditor/RichTextEditor engine 化）规模大，建议放在 PR-16 之后单独排期；DataTableWithToolbar 状态机抽离仍随 PR-16 一并评估。
