...
<template>
  <div>
    ...
    <Modal v-model:visible="visible" title="基本对话框">
      <p>这是对话框的内容。</p>
...
```vue
<template>
  <Modal v-model:visible="visible" title="自定义页脚">
    <p>这是对话框的内容。</p>
...
<template>
  <div>
    ...
    <Modal v-model:visible="visible" :size="size" title="不同尺寸的对话框">
...
```vue
<template>
  <Modal v-model:visible="visible" title="垂直居中对话框" centered>
    <p>这个对话框垂直居中显示。</p>
  </Modal>
</template>
```
...
```vue
<template>
  <Modal v-model:visible="visible" title="禁用遮罩关闭" :mask-closable="false">
    <p>点击遮罩层不会关闭此对话框。</p>
  </Modal>
</template>
```
...
```vue
<template>
  <Modal v-model:visible="visible" title="关闭时销毁" destroy-on-close>
    <p>关闭对话框时，此内容将被销毁。</p>
  </Modal>
</template>
```
...
| style             | 自定义样式                               | `Record<string, unknown>`                | -         |
| closeAriaLabel    | 关闭按钮 aria-label                      | `string`                                 | `'Close'` |
| showDefaultFooter | （Vue）未提供 footer slot 时渲染默认页脚 | `boolean`                                | `false`   |
| okText            | （Vue）默认确定文案                      | `string`                                 | `'确定'`  |
| cancelText        | （Vue）默认取消文案                      | `string`                                 | `'取消'`  |
...
| onOk            | 点击确定按钮的回调（需由自定义 footer 触发） | `() => void`                 | -      |
...
| close          | 对话框关闭时触发                                   | -                    |
| cancel         | 点击取消或关闭按钮时触发                           | -                    |
| ok             | 点击确定按钮时触发（默认页脚/slot helpers 调用时） | -                    |
...
`footer` 支持 slot props：`{ ok, cancel }`，用于触发对应事件并关闭对话框。
...
:root {
  ...
  --tiger-modal-mask: rgba(0, 0, 0, 0.5);
...
```tsx
<Modal visible={visible} className="custom-modal" title="自定义样式">
  <p>自定义样式的对话框。</p>
</Modal>
```
...
- 标题通过 `aria-labelledby` 与对话框关联（实例级唯一 id）
- 关闭按钮包含 `aria-label="Close"` 属性
- 遮罩层使用 `aria-hidden="true"` 标记
- 支持 `ESC` 键关闭（触发 cancel/onCancel）
