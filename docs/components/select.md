...
| options | 选项数据 | `SelectOption[]` \| `SelectOptionGroup[]` | `[]` | - |
...
| 属性 | 说明 | 类型 | 默认值 |
|------|------|------|--------|
| modelValue (v-model) | 当前选中值（单选为 `string \| number`，多选为 `(string \| number)[]`） | `string` \| `number` \| `(string \| number)[]` | - |
...
| onChange | 值变化时的回调 | `(value: string \| number \| (string \| number)[] \| undefined) => void` | - |
