...
scope:
  ...
  - G: CLI、脚本、发布与维护自动化（G-1~G-5）
  - H: 最终公共内容拆分/合并决策汇总（H-0~H-8）
sources:
  - id: A
    read:
  ...
  - id: G
    read:
      - packages/cli/src（create/add/playground/generate/doctor、templates、fs/logger/validate）
      - scripts/*.mjs（setup/check-env/release-readiness/publish-check/run-examples/api/test validate/utils）
      - 根 package.json scripts、scripts/README.md、tests/core/cli.spec.ts
    validation: release:check 实跑、CLI vitest；静态 scripts/README/package scripts 核对
  - id: H
    read: A、B、C01-C32、F、G 各节「供任务 H 汇总」决策（无新读源，纯汇总）
    validation: prettier --check、git diff --check、来源编号回溯核对（文档类，不跑 quality:release）
...
**结论速览**：C26 目标 vitest/API/type 门禁均通过；scale、path、stack、legend、tooltip formatter、pointer move scheduler、Bar/Scatter 单点交互和双端基础视觉增强已有覆盖。**无 P1**。需处理项集中在公开 props 与实际消费、交互契约和大数据策略：① LineChart/AreaChart 的单系列颜色 props 公开但双端未消费（P2）；② Line/Area 点级 tooltip/hover/click 绕过 `hoverable`，且点级没有键盘激活路径（P2）；③ ROADMAP 关注的降采样在 C26 无 API/helper/spec 落点（P3）；④ React BarChart 的 bar rect 缺 role/aria-label，和 Vue Bar / 双端 Scatter 不对称（P3）。

---

#### C26-1 LineChart / AreaChart 单系列颜色 props 公开但双端未消费 — **P2**

**发现问题**

- 🟠 P2｜core [LineChartProps](../packages/core/src/types/chart.ts) 明确公开 `lineColor?: string`（“Line color (for single series)”）与 `pointColor?: string`，AreaChart 公开 `areaColor?: string` 与 `pointColor?: string`。但 React LineChart 解构为 `_lineColor` / `_pointColor` 后零使用；React AreaChart 解构为 `_areaColor` / `_pointColor` 后零使用。
- 🟠 P2｜Vue LineChart / AreaChart 同样声明 `lineColor` / `areaColor` / `pointColor` props，但 `palette = resolveChartPalette(props.colors)`，series color 只取 `series.color ?? palette[...]`，点色只取 `series.pointColor ?? color`；单系列 `data` 经 `resolveSeriesData(series, data)` 包装时没有把 `lineColor` / `areaColor` / `pointColor` 下沉到默认 series。
- ℹ️ BarChart 与 ScatterChart 对应单项颜色路径是健康的：BarChart 使用 `resolveChartPalette(colors, barColor)`，ScatterChart 使用 `resolveChartPalette(colors, pointColor)`，双端 spec 也覆盖自定义 `barColor` / `pointColor`。Line/Area 双端 spec 只覆盖 per-series `color`，未覆盖单系列 `lineColor` / `areaColor` / `pointColor`。

**公共内容决策**：走 API 收敛。保留公开 props，并在 Line/Area 的 `resolveSeriesData` 默认 series 或 palette fallback 中真正消费：Line 用 `lineColor` 作为单系列线/默认点色 fallback，Area 用 `areaColor` 作为单系列线/填充 fallback，`pointColor` 覆盖点色。补 React/Vue 单系列颜色 spec，避免 generated props 继续承诺无效能力。

**建议修复顺序**：P2。优先修公开 props 漂移，成本低且避免文档/API 误导。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/LineChart.spec.tsx tests/react/AreaChart.spec.tsx tests/vue/LineChart.spec.ts tests/vue/AreaChart.spec.ts
...
#### C26-2 Line/Area 点级交互绕过 `hoverable`，且点级无键盘激活路径 — **P2**

**发现问题**

- 🟠 P2｜LineChart / AreaChart 的 series 级交互走 `useChartInteraction`，受 `hoverable` / `selectable` 控制；但点级 `handlePointMouseEnter` 无条件设置 `hoveredPointInfo`、更新 tooltip 坐标并触发 `onPointHover` / `point-hover`，`ChartTooltip` 只判断 `showTooltip && hoveredPointInfo !== null`。因此默认 `hoverable=false` 时，LineChart 仍会因默认 `showPoints=true` + `showTooltip=true` 在点 hover 时显示 tooltip；AreaChart 在 `showPoints=true` 时同理。
- 🟠 P2｜Line/Area 点 `<circle>` 只有鼠标 enter/move/leave/click，没有 `tabIndex`、`role`、`aria-label` 或 Enter/Space 激活。键盘路径只存在于 series group，并选择/点击 series，不会触发 `onPointClick` / `point-click`。这与 Scatter 点（双端 role/aria + keydown）和 Bar（至少 Vue 端 role/aria + keydown）不对称。
- ℹ️ 现有双端 AreaChart spec 覆盖 series 键盘与 point 鼠标 click/hover；LineChart spec 未覆盖交互；没有测试锁定「`hoverable=false` 时点 tooltip 是否应出现」或点级键盘激活。

**公共内容决策**：明确 Line/Area 的点级 tooltip 与 `hoverable` 关系。建议与 Bar/Scatter 收敛：tooltip/hover callback 受 `hoverable` 控制；点级 click 受 `selectable` 或显式 point click callback 控制，并为可交互点补 `role="button"`、`tabIndex=0`、`aria-label`、Enter/Space 触发。

**建议修复顺序**：P2。与 C26-1 分批也可；先定契约，再补双端交互 spec。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/LineChart.spec.tsx tests/react/AreaChart.spec.tsx tests/vue/LineChart.spec.ts tests/vue/AreaChart.spec.ts tests/core/chart-interaction.spec.ts
```

---

#### C26-3 降采样没有 API / helper / spec 落点，大数据路径仍全量渲染 — **P3**

**发现问题**

- 🟢 P3｜ROADMAP C26 明确把“降采样”列为重点，但 `LineChartProps` / `AreaChartProps` / `ScatterChartProps` / `BarChartProps` 没有 sampling/downsample/decimation 相关 props；core `utils/chart/*` 也没有采样 helper；定向 spec 无相关覆盖。
- 🟢 P3｜Line/Area 对每个点都映射并生成 path / circle；Bar/Scatter 对每个 datum 渲染 rect / circle/path，Scatter 还按 palette 生成 radialGradient defs，Bar gradient 按 bar 生成 linearGradient defs。当前设计适合中小数据，未定义大数据渲染阈值、抽样算法、tooltip 索引回映射或可关闭的渐变批量策略。
- ℹ️ 这不是当前 public API 违约，因为没有公开降采样字段；但它是 ROADMAP 指定扫描项，建议作为后续性能能力明确建模。

**公共内容决策**：先以 core helper 定义抽样契约，再接框架组件。建议新增可测试的 `downsampleSeriesData` / `downsampleXYData` 纯函数，明确输入顺序、保峰值策略、阈值、tooltip 原始索引映射；组件层新增 opt-in prop，默认保持现有全量渲染。

**建议修复顺序**：P3。等 C25 基础图表子组件扫描完成后再定复用边界，避免在单个图表组件里各自发明采样。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/core/chart-utils.spec.ts tests/react/LineChart.spec.tsx tests/react/AreaChart.spec.tsx tests/react/ScatterChart.spec.tsx tests/vue/LineChart.spec.ts tests/vue/AreaChart.spec.ts tests/vue/ScatterChart.spec.ts
```

---

#### C26-4 React BarChart bar rect 缺少 role/aria-label，与 Vue Bar / Scatter 不对称 — **P3**

**发现问题**

- 🟢 P3｜React BarChart 的 `<rect>` 绑定 hover/click/keyboard handlers 和 `tabIndex={selectable ? 0 : undefined}`，但没有 `role` 或 `aria-label`；选择态打开时键盘可聚焦但读屏名称缺失。Vue BarChart 同位置已有 `role: props.selectable ? 'button' : 'img'` 与 `'aria-label': bar.datum.label ?? String(bar.datum.x)`。
- 🟢 P3｜ScatterChart 双端点元素均有 role/aria-label；因此 C26 内的点/柱可访问语义目前是 React BarChart 最弱。现有 axe smoke test 能通过，但不会覆盖“每个可交互图元是否有稳定名称”的行为断言。
- ℹ️ Line/Area 点级 a11y 已在 C26-2 作为 P2 覆盖；本项只记录 BarChart 双端不对称。

**公共内容决策**：React BarChart 对齐 Vue BarChart 与 ScatterChart。可交互 bar 用 `role="button"`，非交互但可读图元用 `role="img"` 或保持无 role 但至少提供稳定 `aria-label`；补 React spec 断言 selectable bar 的 role/name。

**建议修复顺序**：P3。可与 C26-2 的 chart a11y 修复批次合并。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/BarChart.spec.tsx tests/vue/BarChart.spec.ts tests/react/ScatterChart.spec.tsx tests/vue/ScatterChart.spec.ts
```

---

#### C26 健康面：scale/path/shared tooltip/legend 复用基线较稳

- ✅ `createLinearScale` / `createPointScale` / `createBandScale`、`getNumberExtent`、`getChartAxisTicks`、`createLinePath`、`createAreaPath`、`stackSeriesData` 均有 core spec 覆盖；C26 组件实际复用这些 helper，而不是各自手写路径和坐标系。
- ✅ `buildChartSeriesKeys` 已覆盖 name/id/key 稳定性，Line/Area 双端 spec 覆盖 named series 插入后 gradient/path id 保持稳定，避免 React/Vue 渲染重排后 url(#id) 漂移。
- ✅ Bar/Scatter 走共享 `useChartInteraction` + core pointer scheduler，hover/select/legend/tooltip 的主要状态机双端对称；C26 定向测试覆盖 Bar negative values、barMinHeight/barMaxWidth、Scatter point styles、controlled hovered/selected opacity。
...
### 任务 C / C27：径向图表组扫描结果（2026-06-27）

**扫描范围**：PieChart / DonutChart / RadarChart / GaugeChart 全链路——core 类型 [chart.ts](../packages/core/src/types/chart.ts) 的 C27 props，core 工具 [chart-utils.ts](../packages/core/src/utils/chart-utils.ts)、[chart-shared.ts](../packages/core/src/utils/chart-shared.ts)、[chart-interaction.ts](../packages/core/src/utils/chart-interaction.ts)、[gauge-chart-utils.ts](../packages/core/src/utils/gauge-chart-utils.ts) 与 `utils/chart/{scale,path,format,color}.ts`，React 实现 `components/{PieChart,DonutChart,RadarChart,GaugeChart}.tsx` 与 [useChartInteraction.ts](../packages/react/src/hooks/useChartInteraction.ts)，Vue 实现 `components/{PieChart,DonutChart,RadarChart,GaugeChart}.ts` 与 [useChartInteraction.ts](../packages/vue/src/composables/useChartInteraction.ts)，`tests/core/{chart-utils,chart-shared,chart-interaction,gauge-chart-utils}.spec.ts`、`tests/{react,vue}/{PieChart,DonutChart,RadarChart,GaugeChart}.spec.*`，以及 generated references（component-index、shared props charts、examples charts）和双端 demo。C24 VirtualList / InfiniteScroll 未在本轮执行；C25/C26 已分别覆盖基础图表子组件与笛卡尔图表。

**结论速览**：C27 目标 vitest/API/type 门禁均通过；Pie/Donut 角度计算复用 core `getPieArcs` / `createPieArcPath`，Radar 角度/点位/series key 与 Gauge rAF 动画均有定向覆盖。**无 P1**。需处理项集中在公开 props 与极值契约：① Gauge 公开继承 tooltip props 但双端没有 tooltip 实现（P2）；② Donut `animated` 是公开 no-op，Vue Pie/Donut 未消费 `legendFormatter`（P2）；③ Gauge `min === max`、`tickCount=0`、小尺寸/大 `arcWidth` 等极值会生成 NaN/非法几何（P2）；④ Radar 点级 tooltip 只有鼠标路径，React Pie/Donut selectable slice 缺 role/name（P3）；⑤ Pie/Donut mixed 0/负值数据仍渲染空 `path`，label/legend/交互计数会包含不可见项（P3）。

---

#### C27-1 GaugeChart 公开继承 `ChartTooltipProps`，但双端组件没有 tooltip 实现 — **P2**

**发现问题**

- 🟠 P2｜core [GaugeChartProps](../packages/core/src/types/chart.ts) `extends BaseChartProps, ChartTooltipProps`，因此公共类型承诺了 `showTooltip` / `tooltipFormatter`；generated [shared/props/charts.md](../skills/tigercat/references/shared/props/charts.md) 也把 GaugeChart 归入 chart props 面。
- 🟠 P2｜React [GaugeChart.tsx](../packages/react/src/components/GaugeChart.tsx) 的 props 解构只接 `valueFormatter` / `label` / `segments` / `trackColor` / `color` / `gradient` 等字段，没有导入或渲染 `ChartTooltip`，也没有读取 `showTooltip` / `tooltipFormatter`。
- 🟠 P2｜Vue [GaugeChart.ts](../packages/vue/src/components/GaugeChart.ts) 的 runtime props 同样没有 `showTooltip` / `tooltipFormatter`，setup 内只渲染 track/value arc/ticks/needle/value text；这会造成 React 类型层可传但无效果、Vue 运行时/类型面更窄的双端不一致。

**公共内容决策**：走 API 收敛。二选一：若 Gauge 需要 tooltip，则双端补 `showTooltip` / `tooltipFormatter` runtime props、hover/focus 命中区域和 `ChartTooltip` 渲染；若不需要 tooltip，则从 core `GaugeChartProps` 移除 `ChartTooltipProps`，并更新 generated references/API baseline。倾向补实现，因为 Gauge 已有 value arc/segments 可作为 tooltip 数据源。

**建议修复顺序**：P2。先定 Gauge tooltip 是否属于 public contract，再同步 core 类型、双端 runtime props、tests 与 references。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/GaugeChart.spec.tsx tests/vue/GaugeChart.spec.ts tests/core/gauge-chart-utils.spec.ts
corepack pnpm api:validate
corepack pnpm types:check
```
...
#### C27-2 Pie/Donut public props 漂移：Vue `legendFormatter` 缺口 + Donut `animated` 公开 no-op — **P2**

**发现问题**

- 🟠 P2｜core [PieChartProps](../packages/core/src/types/chart.ts) 公开 `legendFormatter?: (datum, index) => string`。React [PieChart.tsx](../packages/react/src/components/PieChart.tsx) 在 `buildChartLegendItems` 中消费 `legendFormatter`；Vue [PieChart.ts](../packages/vue/src/components/PieChart.ts) runtime props 未声明 `legendFormatter`，legend label 固定为 `d.label ?? value`。
- 🟠 P2｜DonutChart 继承 PieChartProps。React [DonutChart.tsx](../packages/react/src/components/DonutChart.tsx) 没有解构/转发 `legendFormatter` 到 PieChart；Vue [DonutChart.ts](../packages/vue/src/components/DonutChart.ts) 也没有声明或转发该 prop，因此 Donut 双端都无法兑现 core 继承来的 legend formatter。
- 🟠 P2｜core [DonutChartProps](../packages/core/src/types/chart.ts) 公开 `animated?: boolean`，React 组件解构为 `_animated = false` 后零消费；Vue 组件声明 `animated` prop，但 setup/render 没有任何消费。当前双端测试只覆盖默认 shadow/center/custom color，没有覆盖 animated 行为。

**公共内容决策**：走 API 收敛。Pie/Donut 的 `legendFormatter` 应与 React Pie 现状对齐，补 Vue Pie runtime prop，并让 Donut 双端转发；`animated` 要么实现入口动画（且尊重 reduced motion），要么从 Donut public API 降级/移除并更新 references，避免 no-op 承诺继续扩散。

**建议修复顺序**：P2。先补 formatter 这类低风险契约缺口；`animated` 与图表动画策略一起定，避免 Donut 单点发明动画行为。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/PieChart.spec.tsx tests/vue/PieChart.spec.ts tests/react/DonutChart.spec.tsx tests/vue/DonutChart.spec.ts
corepack pnpm api:validate
corepack pnpm types:check
```
...
#### C27-3 Gauge 极值未规范化：`min === max` / `tickCount=0` / 小半径大 `arcWidth` 会生成 NaN 或非法几何 — **P2**

**发现问题**

- 🟠 P2｜[valueToGaugeAngle](../packages/core/src/utils/gauge-chart-utils.ts) 直接计算 `(value - min) / (max - min)`；`min === max` 时返回 NaN。当前 [gauge-chart-utils.spec.ts](../tests/core/gauge-chart-utils.spec.ts) 还把“returns NaN due to division by zero”写成断言，说明异常行为已被测试锁住。
- 🟠 P2｜[computeGaugeTicks](../packages/core/src/utils/gauge-chart-utils.ts) 对 `tickCount=0` 做 `i / tickCount`，会得到 NaN tick 坐标和值；现有 spec 只断言 tick 数量为 1，没有断言坐标/label 有限。React/Vue Gauge 默认会把这些坐标传给 `<line>`/`<text>`。
- 🟠 P2｜Gauge 双端半径均为 `Math.min(innerRect.width, innerRect.height) / 2 - 4`，没有 clamp；当容器很小或 `arcWidth >= radius` 时，`createGaugeArcPath` 的 inner radius 可能为 0/负数，needle length 也可能为负。ROADMAP C27 明确关注“空值和极值”，这类边界目前没有组件级测试。

**公共内容决策**：抽 core normalization。建议新增/收敛 `normalizeGaugeScale` 或在现有 helpers 内处理：`max <= min` 时选择稳定 fallback（例如 startAngle 或 endAngle，需定契约）、`tickCount` clamp 到 `>= 1` 或明确单 tick 语义、`radius/arcWidth/needleLength` clamp 到非负有限值。同步改掉当前 NaN 断言。

**建议修复顺序**：P2。先改 core helper 和 spec，再让 React/Vue 继续复用同一几何契约。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/core/gauge-chart-utils.spec.ts tests/react/GaugeChart.spec.tsx tests/vue/GaugeChart.spec.ts
```

---

#### C27-4 径向图表点/切片 a11y 不对称：Radar 点仅鼠标路径，React Pie/Donut selectable slice 缺 role/name — **P3**

**发现问题**

- 🟢 P3｜RadarChart 双端 series group 有 selectable 键盘路径，但点级 tooltip 只绑定 `onMouseEnter` / `onMouseMove` / `onMouseLeave`；`circle[data-radar-point]` 没有 `role`、`tabIndex`、`aria-label` 或键盘触发。用户只能用鼠标看到点级 tooltip，键盘只能选中 series。
- 🟢 P3｜React [PieChart.tsx](../packages/react/src/components/PieChart.tsx) 在 selectable 时给 slice 设置 `tabIndex=0` 和 `onKeyDown`，但没有 `role` / `aria-label`；Vue [PieChart.ts](../packages/vue/src/components/PieChart.ts) 已设置 `role: selectable ? 'button' : 'img'` 与 `aria-label`。Donut 复用 Pie，因此 React Donut 继承该缺口。
- ℹ️ 当前 a11y smoke test 通过，但它不会断言“可聚焦 SVG 图元是否有稳定名称”。C26 已记录 React BarChart 同类缺口，本项可与图表 a11y 批次合并。

**公共内容决策**：框架层对齐语义。React Pie/Donut 对齐 Vue slice 的 role/name；Radar 点若保留点级 tooltip，应提供可聚焦/可命名的点级图元，或明确 tooltip 只属于 pointer enhancement 并避免让点看起来是主要交互目标。

**建议修复顺序**：P3。可与 C26-2/C26-4 的图表 a11y 修复一起处理，统一 role/tabIndex/aria/key 规则。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/PieChart.spec.tsx tests/react/DonutChart.spec.tsx tests/react/RadarChart.spec.tsx tests/vue/PieChart.spec.ts tests/vue/DonutChart.spec.ts tests/vue/RadarChart.spec.ts
```

---

#### C27-5 Pie/Donut mixed 0/负值数据会保留不可见 slice 节点，空值语义不够明确 — **P3**

**发现问题**

- 🟢 P3｜[getPieArcs](../packages/core/src/utils/chart/path.ts) 会把每个 datum 的 value clamp 为 `Math.max(0, value)`；当 total > 0 时，0/负值 datum 仍返回一条 arc geometry，只是 `sliceAngle=0`。随后 `createPieArcPath` 对 `endAngle <= startAngle` 返回空字符串。
- 🟢 P3｜React/Vue Pie 直接对 `arcs` map 渲染 `path[data-pie-slice]`，因此 mixed 数据如 `[{value:-5},{value:10}]` 会生成一个 `d=""` 的不可见 slice；label、legend、hover/select index 仍按原始 datum 计数。Donut 复用 Pie，同样继承。
- ℹ️ core spec 覆盖“全 0 返回 []”和“负值被 clamp”，但组件 spec 只测空数组，没有覆盖 mixed 0/负值时 DOM 节点、legend、label 与交互索引应如何表现。

**公共内容决策**：明确径向图的空值契约。建议在 core arc 层提供 `includeZeroSlices` 或默认过滤 0 值 arc，并同步定义 legend 是否保留 0 值项；如果决定保留，应避免渲染空 `path` 并给 label/tooltip 明确 0% 语义。

**建议修复顺序**：P3。先补 core + 组件测试锁定期望，再改 Pie/Donut 渲染。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/core/chart-utils.spec.ts tests/react/PieChart.spec.tsx tests/react/DonutChart.spec.tsx tests/vue/PieChart.spec.ts tests/vue/DonutChart.spec.ts
```

---

#### C27 健康面：径向几何与双端基础视觉复用整体可用

- ✅ Pie/Donut 共享 core `getPieArcs` / `createPieArcPath` / `computePieLabelLine`，并通过 `getStableChartGradientPrefix` 避免 gradient id 碰撞；双端基础 slice、label、outside leader line、gradient/shadow、tooltip/legend smoke 均有覆盖。
- ✅ Radar 双端复用 `resolveSeriesData`、`buildChartSeriesKeys`、`getRadarAngles`、`getRadarPoints`、`createPolygonPath` 与 `resolveMultiSeriesTooltipContent`；多 series、circle grid、split area、level label、gradient/stroke/point gradient、series keyboard select 均有定向测试。
- ✅ Gauge 双端复用同一 `valueToGaugeAngle` / `computeGaugeTicks` / `createGaugeArcPath` / `createGaugeNeedlePath` / `createGaugeAnimation`，并覆盖 custom formatter、segments、gradient、className、title/desc、value clamp above max。
...
### 任务 C / C29：复合内容组扫描结果（2026-06-27）

**扫描范围**：ActivityFeed / ChatWindow / CommentThread 全链路——core 类型 [composite.ts](../packages/core/src/types/composite.ts) 的 C29 props，core 工具 [activity-feed-utils.ts](../packages/core/src/utils/activity-feed-utils.ts)、[chat-window-utils.ts](../packages/core/src/utils/chat-window-utils.ts)、[comment-thread-utils.ts](../packages/core/src/utils/comment-thread-utils.ts)，React 实现 `components/{ActivityFeed,ChatWindow,CommentThread}.tsx`，Vue 实现 `components/{ActivityFeed,ChatWindow,CommentThread}.ts`，底层 [Timeline](../packages/react/src/components/Timeline.tsx) / [Timeline](../packages/vue/src/components/Timeline.ts) 对照，`tests/{react,vue}/{ActivityFeed,ChatWindow,CommentThread}.spec.*`、`tests/core/{composite-a11y-roles.spec.tsx,modern-theme-interaction.spec.ts}`，以及 generated references（component-index、shared props composite、examples composite、release notes）和双端 demo。C24 VirtualList / InfiniteScroll 与 C30 FormWizard / TaskBoard / Kanban 未在本轮执行。

**结论速览**：C29 目标 vitest/API/type 门禁均通过；ChatWindow 已按 `virtual` 走 VirtualList，三组件均复用基础组件并有空态与基础 a11y smoke 覆盖。**无 P1**。需处理项集中在 public prop/事件契约和长列表能力：① ChatWindow `statusVariant` 是公开 no-op（P2）；② CommentThread Vue 类型仍暴露 React 风格 `onExpandedChange`，但运行时只发 `update:expandedKeys`（P2）；③ ActivityFeed 依赖 Timeline 全量渲染，没有 C29 关注的长列表策略（P3）；④ custom render/slot、action callback 与 statusVariant 缺少双端断言（P3）。

---

#### C29-1 ChatWindow `statusVariant` 公开但双端没有运行时效果 — **P2**

**发现问题**

- 🟠 P2｜core [ChatWindowProps](../packages/core/src/types/composite.ts) 公开 `statusVariant?: BadgeVariant`，注释为状态栏 variant；generated props 将 ChatWindow 归入 composite public 面。
- 🟠 P2｜React [ChatWindow.tsx](../packages/react/src/components/ChatWindow.tsx) 解构 `statusVariant: _statusVariant = 'info'` 后零消费；状态栏只用固定 italic/muted class 渲染 `statusText`。
- 🟠 P2｜Vue [ChatWindow.ts](../packages/vue/src/components/ChatWindow.ts) 声明 `statusVariant` prop，但 setup/render 内没有读取；双端 spec 只覆盖 `statusText`、消息 status 与 VirtualList，不覆盖 status bar variant。

**公共内容决策**：走 API 收敛。若状态栏需要语义色，则复用 Badge variant class 或抽 core `getChatStatusBarClasses(statusVariant)`，双端同一规则消费；若状态栏只保留文本，则从 public type / generated references 移除 `statusVariant`，避免 no-op prop 继续扩散。

**建议修复顺序**：P2。先决定保留还是降级 API；保留时补 React/Vue status bar variant 断言。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/ChatWindow.spec.tsx tests/vue/ChatWindow.spec.ts
corepack pnpm api:validate
corepack pnpm types:check
```
...
#### C29-2 CommentThread Vue 类型暴露 `onExpandedChange`，运行时只发 `update:expandedKeys` — **P2**

**发现问题**

- 🟠 P2｜core [CommentThreadProps](../packages/core/src/types/composite.ts) 公开 `onExpandedChange?: (keys) => void`，React [CommentThread.tsx](../packages/react/src/components/CommentThread.tsx) 在 `updateExpandedKeys` 中调用该回调。
- 🟠 P2｜Vue [CommentThread.ts](../packages/vue/src/components/CommentThread.ts) 的 `VueCommentThreadProps extends CoreCommentThreadProps`，因此类型面仍继承 `onExpandedChange`；但组件 `emits` 只有 `update:expandedKeys`，`toggleExpanded` 只 `emit('update:expandedKeys', next)`，不会消费或转发 `onExpandedChange`。
- 🟠 P2｜generated [shared/props/composite.md](../skills/tigercat/references/shared/props/composite.md) 仍在 Events/callback props 里列 `onExpandedChange?`，但同页 Note 和 [release.md](../skills/tigercat/references/release.md) 又说明 Vue 使用 `update:expandedKeys` / `v-model:expanded-keys`。这会让 Vue TS/文档读者同时看到两个展开事件契约。

**公共内容决策**：按 release note 收敛 Vue 契约。Vue 类型应 `Omit<CoreCommentThreadProps, 'onExpandedChange'>` 后显式描述 `update:expandedKeys` 事件，generated docs 区分 React callback 与 Vue model event；React 继续保留历史 `onExpandedChange`。

**建议修复顺序**：P2。与 generated docs/API 类型批次一起处理，避免只改文档或只改类型造成新的不一致。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/CommentThread.spec.tsx tests/vue/CommentThread.spec.ts
corepack pnpm api:validate
corepack pnpm types:check
```
...
#### C29-3 ActivityFeed 长列表仍全量渲染，缺少 VirtualList / 分页 / 折叠策略入口 — **P3**

**发现问题**

- 🟢 P3｜ROADMAP C29 明确关注“列表性能”。ActivityFeed 双端先用 core `buildActivityGroups` / `toActivityTimelineItems` 分组，再把每组 `items` 交给 Timeline；ActivityFeed 自身没有 `virtual`、`itemHeight`、`limit`、分页或折叠 props。
- 🟢 P3｜底层 React/Vue Timeline 都对 `processedItems` 做全量 `map` 渲染；没有复用 C24 的 VirtualList / InfiniteScroll，也没有阈值或性能提示。分组活动流在几百/几千条动态时会按所有节点生成 DOM。
- ℹ️ ChatWindow 已有 `virtual` / `virtualItemHeight` / `virtualHeight`，并有双端 spec 断言 200 条消息只渲染小窗口；ActivityFeed 没有同类能力或测试。

**公共内容决策**：不要在 ActivityFeed 内临时手写虚拟滚动。建议先等 C24 VirtualList / InfiniteScroll 扫描定边界，再决定 ActivityFeed 是新增 opt-in `virtual` 契约、暴露 `renderGroup` 让业务侧外层分页，还是只在 docs 中声明适用数据规模。

**建议修复顺序**：P3。放到 C24 后处理；若新增虚拟能力，应同时补分组 header、高度估算和空/加载态测试。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/ActivityFeed.spec.tsx tests/vue/ActivityFeed.spec.ts tests/react/Timeline.spec.tsx tests/vue/Timeline.spec.ts
```

---

#### C29-4 自定义渲染、action callback 与复合事件覆盖不足 — **P3**

**发现问题**

- 🟢 P3｜ChatWindow React 覆盖 `renderMessage`，Vue 覆盖 `message` slot；但这类 framework-specific extension 没有在 generated examples 中展示，也没有覆盖 custom content 与 `virtual` 同时开启的路径。
- 🟢 P3｜ActivityFeed 双端 spec 覆盖分组、空态、loading 与 `showGroupTitle=false`，但未断言 `showAvatar=false`、`showTime=false`、`renderItem` / `item` slot、`renderGroupHeader` / `groupTitle` slot、action `onClick` 与 disabled action 行为。
- 🟢 P3｜CommentThread 双端 spec 覆盖 reply、load more、like count 和空态；但 `onLike` / `like`、`onMore` / `more`、custom `actions` + `onAction` / `action`、受控 `expandedKeys` 不更新内部状态等关键交互没有双端成对断言。

**公共内容决策**：补测试优先于改实现。C29 这三个复合组件已经组合了多种基础组件，后续修复 public prop 或事件契约前，应先把 custom render/slot、action callback、受控/非受控展开路径的双端行为锁住。

**建议修复顺序**：P3。与 C29-1/C29-2 修复同批补最小断言；不需要改变 public API。

**目标验证命令**：

```bash
corepack pnpm vitest run tests/react/ActivityFeed.spec.tsx tests/vue/ActivityFeed.spec.ts tests/react/ChatWindow.spec.tsx tests/vue/ChatWindow.spec.ts tests/react/CommentThread.spec.tsx tests/vue/CommentThread.spec.ts
```

---

#### C29 健康面：复合组件的基础依赖复用与空态/a11y 基线可用

- ✅ ChatWindow 双端复用 Avatar、Textarea/Input、Button、VirtualList、Empty；`virtual` 开启后确实走 VirtualList，目标 spec 覆盖 200 条消息窗口化渲染、Enter/Shift+Enter、`clearOnSend=false`、custom message content、空态和 axe smoke。
- ✅ ActivityFeed 双端复用 Timeline、Avatar、Tag、Card、Text、Link、Loading，并把分组/排序沉到 core `buildActivityGroups`，Timeline bridge 通过 `ActivityTimelineItem.activity` 避免 renderer 内不安全散落 cast；loading state 有 `aria-busy` 覆盖。
- ✅ CommentThread 双端复用 `buildCommentTree` / `clipCommentTreeDepth`、Avatar、Tag、Button、Textarea、Text；基础嵌套、maxDepth、maxReplies/load more、reply submit/cancel、empty state、feed role/name 均有定向测试。
...
### 任务 C / C30：工作流复合组扫描结果（2026-06-27）

**扫描范围**：FormWizard、TaskBoard、Kanban 的全链路——core 类型 [composite.ts](../packages/core/src/types/composite.ts) 与 [kanban.ts](../packages/core/src/types/kanban.ts)，core 工具 [form-wizard-utils.ts](../packages/core/src/utils/form-wizard-utils.ts)、[task-board-utils.ts](../packages/core/src/utils/task-board-utils.ts)、[task-board-drag.ts](../packages/core/src/utils/task-board-drag.ts)、[kanban-utils.ts](../packages/core/src/utils/kanban-utils.ts)、[locale-utils.ts](../packages/core/src/utils/locale-utils.ts)，React 实现 [FormWizard.tsx](../packages/react/src/components/FormWizard.tsx)、[TaskBoard.tsx](../packages/react/src/components/TaskBoard.tsx)、[Kanban.tsx](../packages/react/src/components/Kanban.tsx)，Vue 实现 [FormWizard.ts](../packages/vue/src/components/FormWizard.ts)、[TaskBoard.ts](../packages/vue/src/components/TaskBoard.ts)、[Kanban.ts](../packages/vue/src/components/Kanban.ts)，tests/{core,react,vue} 14 个定向 spec / custom-text spec，examples `{FormWizard,TaskBoard,Kanban}Demo`，以及 generated references（component-index、shared props composite/advanced、examples composite/advanced、api-summary）。

**结论速览**：C30 的基础导航、step validation、TaskBoard HTML5/touch/keyboard drag controller、WIP enforcement、双端 locale/labels 基线和 a11y smoke 均有覆盖，目标 vitest/API/type 门禁通过。**无 P1**。需处理项集中在 public surface 与文案/文档契约：① core `KanbanProps` 与 generated refs 声明 swimlane 能力，但双端 `Kanban` 未暴露也未实现（P2）；② TaskBoard 新增列文案在 React/Vue 不一致，且无法独立本地化（P2）；③ generated props 对 C30 核心能力覆盖不足（P3）；④ FormWizard 受控/初始越界 current 缺少统一 clamp 契约与测试（P3）。

---

#### C30-1 Kanban swimlane 能力停在 core 类型与工具层，双端组件未实现 — **P2**

**发现问题**

- 🟠 P2｜[KanbanProps](../packages/core/src/types/kanban.ts) 扩展 `TaskBoardProps` 并公开 `swimlanes?: KanbanSwimlane[]` 与 `swimlaneField?: string`；[kanban-utils.ts](../packages/core/src/utils/kanban-utils.ts) 也实现了 `groupBySwimlane(cards, swimlanes, fieldName)`，core spec 覆盖了分组逻辑。
- 🟠 P2｜React [Kanban.tsx](../packages/react/src/components/Kanban.tsx) 手写 `KanbanProps`，只透传 TaskBoard 相关字段并设置 `showCardCount=true`、`allowAddCard=true`；没有声明或消费 `swimlanes` / `swimlaneField`。Vue [Kanban.ts](../packages/vue/src/components/Kanban.ts) 同样只把 TaskBoard props 转发给内部 TaskBoard，没有泳道布局或按字段分组渲染。
- 🟠 P2｜generated [shared/props/advanced.md](../skills/tigercat/references/shared/props/advanced.md) 已把 `swimlanes?` / `swimlaneField?` 写成 Kanban props，api baseline 也显示 core `KanbanProps` 只包含这两个增量字段；但 React/Vue spec、examples 和 demo 都没有任何 swimlane 用例。用户按文档传入这些 props 会得到静默无效能力。

**公共内容决策**：Kanban 继续保持 TaskBoard 薄封装，除非正式实现 swimlane 布局；不要复制 TaskBoard 拖拽逻辑。下一步必须二选一：实现泳道时让 core `groupBySwimlane` 驱动布局并补双端 props/spec/demo；若暂不实现，则从 public/docs 中降级或移除 `swimlanes` / `swimlaneField`，避免半公开 API。

**建议修复顺序**：P2。先做产品/API 决策，再同步 core type、React/Vue props、generated references 和双端测试。

**目标验证命令**：

```bash
pnpm vitest run tests/core/kanban-utils.spec.ts tests/react/Kanban.spec.tsx tests/vue/Kanban.spec.ts
pnpm api:validate
pnpm types:check
```
...
#### C30-2 TaskBoard 新增列文案双端不一致，且无法独立本地化 — **P2**

**发现问题**

- 🟠 P2｜[TigerLocaleTaskBoard](../packages/core/src/types/locale.ts) 只有 `emptyColumnText`、`addCardText`、`wipLimitText`、`dragHintText`、`boardAriaLabel`，没有 `addColumnText`；[getTaskBoardLabels](../packages/core/src/utils/locale-utils.ts) 因此只能返回新增卡片文案。
- 🟠 P2｜React [TaskBoard.tsx](../packages/react/src/components/TaskBoard.tsx) 在 `allowAddColumn` 分支直接渲染 `+ Add column`，绕过 `labels` / `locale` / `ConfigProvider`；Vue [TaskBoard.ts](../packages/vue/src/components/TaskBoard.ts) 的新增列按钮则渲染 `+ ${labels.addCardText}`，会显示成 `+ Add task` / `+ 添加任务`。
- 🟠 P2｜现有 tests 锁定了这个不一致：React 查找 `+ Add column`，Vue 注释写明 add-column button 复用 `addCardText`。示例的 custom labels 只覆盖空列和新增卡片，未暴露新增列文案无法单独覆盖的问题。

**公共内容决策**：TaskBoard 文案能力应统一放在 `TigerLocaleTaskBoard`，新增列使用独立 `addColumnText`；React/Vue 都通过 `getTaskBoardLabels` 渲染，不在组件内硬编码，也不复用 `addCardText`。

**建议修复顺序**：P2。先扩展 locale type/defaults/内置语言，再改双端渲染和 custom-text / TaskBoard / Kanban spec，最后由生成器更新 references。

**目标验证命令**：

```bash
pnpm vitest run tests/core/custom-text-labels.spec.ts tests/react/TaskBoard.spec.tsx tests/vue/TaskBoard.spec.ts tests/react/Kanban.spec.tsx tests/vue/Kanban.spec.ts tests/react/custom-text.spec.tsx tests/vue/custom-text.spec.ts
pnpm api:validate
pnpm types:check
```
...
#### C30-3 generated props 对工作流复合组核心能力覆盖不足 — **P3**

**发现问题**

- 🟢 P3｜generated [shared/props/composite.md](../skills/tigercat/references/shared/props/composite.md) 仅列 `FormWizardProps · 3/19 props` 和 `TaskBoardProps · 3/20 props`，FormWizard 的 `beforeNext`、`showActions`、`labels`、`autoSave`、`onFinish`，TaskBoard 的 WIP、filter/hidden、render hooks、labels、add-column、move validators 等核心能力都没有进入 props 表。
- 🟢 P3｜generated [shared/props/advanced.md](../skills/tigercat/references/shared/props/advanced.md) 对 `KanbanProps` 只列 3/6 props，且其中 swimlane 字段当前双端未实现；实际最常用的 TaskBoard 透传能力、Kanban 默认开启 `showCardCount` / `allowAddCard` 的行为边界没有足够 props 覆盖。
- 🟢 P3｜generated examples 对 FormWizard/TaskBoard/Kanban 只有最小调用摘要，实际 examples 已覆盖受控 current、labels、renderCard/slots、WIP、controlled columns、Kanban WIP 等更能说明工作流组件契约的用法。

**公共内容决策**：通过 `scripts/generate-api-docs.mjs` 或其数据源增强 composite/advanced props 与 examples 摘要；不要手改 generated references。C30-1 / C30-2 的 API 决策落定后再统一更新，避免文档继续记录未实现能力。

**建议修复顺序**：P3。先完成行为/API 收敛，再补生成器覆盖核心 props 与 examples。
...
#### C30-4 FormWizard 受控/初始 current 越界缺少统一契约 — **P3**

**发现问题**

- 🟢 P3｜[form-wizard-utils.ts](../packages/core/src/utils/form-wizard-utils.ts) 提供 `clampStepIndex`，React/Vue 都只在 `setCurrent()` 时调用它；直接传入受控 `current` 或初始 `defaultCurrent` 时，React 的 `currentIndex = current ?? innerCurrent`、Vue 的 `currentIndex = props.current ?? innerCurrent` 不会先 clamp。
- 🟢 P3｜当 `current` / `defaultCurrent` 超出 `steps.length` 时，`currentStep` 为空、内容为空，`isLast` 可能提前为 true；后续点击完成按钮会把越界索引传给 finish 或 change。现有 spec 只覆盖正常 next、skipCondition、beforeNext、autoSave 和 a11y，没有覆盖受控越界、defaultCurrent 越界、steps 缩短后的索引回收。
- ℹ️ 这不影响当前门禁，但作为向导组件的边界契约不明确：要么派生 currentIndex 时统一 clamp，要么文档明确调用方必须保证索引合法。

**公共内容决策**：优先在派生 `currentIndex` 时复用 core `clampStepIndex`，保持 React/Vue 行为一致；如果选择由调用方保证合法，则需要在 generated props/examples 中明确并补测试锁定。

**建议修复顺序**：P3。先补 core/双端契约测试，再调整 React/Vue 派生逻辑或文档。

**目标验证命令**：

```bash
pnpm vitest run tests/core/form-wizard-utils.spec.ts tests/react/FormWizard.spec.tsx tests/vue/FormWizard.spec.ts
```

---

#### C30 公共拆分/合并决策汇总（供任务 H 汇总）

| 项                                | 决策                                                                                     | 优先级 |
| --------------------------------- | ---------------------------------------------------------------------------------------- | ------ |
| Kanban swimlane（C30-1）          | 保持 Kanban 为 TaskBoard 薄封装；泳道能力必须实现或从 public/docs 降级，不保留半公开状态 | **P2** |
| TaskBoard 文案（C30-2）           | 新增列文案进入 `TigerLocaleTaskBoard`，双端统一由 `getTaskBoardLabels` 渲染              | **P2** |
| generated props/examples（C30-3） | 由生成器补 composite/advanced 覆盖，不手改 generated references                          | P3     |
| FormWizard current 边界（C30-4）  | 复用 core clamp 或文档化调用方契约；React/Vue 必须一致                                   | P3     |
...
| 取证                                                                 | 结果                                                                                                                            | 对应发现 |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------- |
| grep `KanbanProps` / `swimlanes` / `groupBySwimlane` / 双端 `Kanban` | core type 和 utils 有 swimlane；React/Vue Kanban 未声明、未消费；spec/demo 无覆盖                                               | C30-1    |
| grep `TigerLocaleTaskBoard` / `+ Add column` / `addCardText`         | locale 无 `addColumnText`；React 硬编码 `+ Add column`；Vue 复用 `addCardText`                                                  | C30-2    |
| generated composite/advanced props                                   | FormWizard/TaskBoard/Kanban 只列少量 props，遗漏工作流关键能力                                                                  | C30-3    |
| grep FormWizard `currentIndex` / `clampStepIndex` / specs            | clamp 只在 `setCurrent()`；受控/初始越界无测试                                                                                  | C30-4    |
| 目标 vitest、`api:validate`、`types:check`                           | ✅ `npx -y pnpm@11.9.0` vitest 14 文件 314 测试通过；`api:validate` 一致性检查通过（0 问题）；`types:check` 全部 props 类型导出 | C30 基线 |

> 本轮 C30 只记录扫描结论和修复建议；未改任何组件源码、core 工具、公共 API、生成器或 generated references（仅本文件 + `docs/ROADMAP.md` 状态标记）。按 ROADMAP「若扫描只更新 Roadmap 文档，不要求跑完整 `pnpm quality:release`，也不运行 `pnpm docs:api`」，本轮用 `npx -y pnpm@11.9.0` 实跑 C30 目标 vitest（14 文件 314 测试通过）、`api:validate` 与 `types:check`，均为只读校验、未改动源码。
...
### 任务 C / C32：交互能力组扫描结果（2026-06-27）

**扫描范围**：Drag/useDrag、Carousel、Resizable、Splitter 的全链路——core 类型 [drag.ts](../packages/core/src/types/drag.ts)、[carousel.ts](../packages/core/src/types/carousel.ts)、[resizable.ts](../packages/core/src/types/resizable.ts)、[splitter.ts](../packages/core/src/types/splitter.ts)，core 工具 [drag.ts](../packages/core/src/utils/drag.ts)、[carousel-utils.ts](../packages/core/src/utils/carousel-utils.ts)、[resizable-utils.ts](../packages/core/src/utils/resizable-utils.ts)、[splitter-utils.ts](../packages/core/src/utils/splitter-utils.ts)，React [useDrag.ts](../packages/react/src/hooks/useDrag.ts) 与 [Carousel.tsx](../packages/react/src/components/Carousel.tsx)、[Resizable.tsx](../packages/react/src/components/Resizable.tsx)、[Splitter.tsx](../packages/react/src/components/Splitter.tsx)，Vue [useDrag.ts](../packages/vue/src/composables/useDrag.ts) 与 [Carousel.ts](../packages/vue/src/components/Carousel.ts)、[Resizable.ts](../packages/vue/src/components/Resizable.ts)、[Splitter.ts](../packages/vue/src/components/Splitter.ts)，tests/{core,react,vue} 14 个定向 spec，examples `{UseDrag,Carousel,Resizable,Splitter}Demo`，以及 generated references（component-index、shared props layout/advanced、examples layout/advanced、api-summary）和 [generate-api-docs.mjs](../scripts/generate-api-docs.mjs)。

**结论速览**：C32 基础交互门禁健康，Drag/useDrag 生命周期、Carousel autoplay/touch/imperative API、Splitter keyboard resize 与 shared `createDocumentDragSession` 都有覆盖，目标 vitest/API/type 门禁通过。**无 P1**。需处理项集中在公开契约和可访问性：① `Splitter.sizes` 文档声明为像素，但 React/Vue 运行语义不一致（P2）；② Resizable handles 只有鼠标交互，缺 keyboard/ARIA resize 契约（P2）；③ Vue `useDrag` 的 options/callbacks/containerId 是创建期快照，和 React hook 更新语义不同（P3）；④ generated references 对 Drag/useDrag 与 C32 高级 props 覆盖不足（P3）。

---

#### C32-1 `Splitter.sizes` 文档声明为像素，但 React/Vue 运行语义不一致 — **P2**

**发现问题**

- 🟠 P2｜core [SplitterProps](../packages/core/src/types/splitter.ts) 与 generated [layout props](../skills/tigercat/references/shared/props/layout.md) 都把 `sizes?: number[]` 描述为每个 pane 的初始像素尺寸。
- 🟠 P2｜React [Splitter.tsx](../packages/react/src/components/Splitter.tsx) 在容器有可测尺寸时把 `controlledSizes` 当比例归一到可用空间：先求 `sum`，再执行 `(s / sum) * total`；测试环境无布局时才回退为原值。Vue [Splitter.ts](../packages/vue/src/components/Splitter.ts) 则在 watch/init 中直接 `paneSizes.value = [...props.sizes]`，与文档的像素语义一致。
- 🟠 P2｜同样传入 `[300, 500]` 时，React 在真实容器中会按 3:5 比例重算，Vue 会使用 300px/500px。现有双端测试只在 happy-dom 或直接样式断言下通过，未覆盖真实容器宽度下的语义差异。

**公共内容决策**：优先按现有 core 文档收敛为像素语义，React 不应在 `sizes` 已提供时自动比例归一；若要支持比例尺寸，应新增明确 prop 或文档化百分比/比例输入，不让同一 `number[]` 双端分叉。

**建议修复顺序**：P2。先补 core/双端测试锁定 `sizes` 像素语义，再调整 React 初始化逻辑，并补真实容器尺寸回归。

**目标验证命令**：

```bash
pnpm vitest run tests/core/splitter-utils.spec.ts tests/react/Splitter.spec.tsx tests/vue/Splitter.spec.ts
pnpm api:validate
pnpm types:check
```
...
#### C32-2 Resizable handles 缺 keyboard/ARIA resize 契约 — **P2**

**发现问题**

- 🟠 P2｜React [Resizable.tsx](../packages/react/src/components/Resizable.tsx) 与 Vue [Resizable.ts](../packages/vue/src/components/Resizable.ts) 的 handle 都是裸 `div[data-handle]`，只绑定 mouse down；没有 `role`、`tabIndex`、`aria-orientation` / `aria-valuenow`，也没有键盘 resize 行为。
- 🟠 P2｜Splitter gutter 已有 `role="separator"`、可 focus 和 Arrow key resize；Resizable 作为同类尺寸交互组件没有等价键盘路径，键盘用户无法触发 resize。
- 🟠 P2｜现有 Resizable a11y 测试只验证 axe 无违规，React 的 “should call onResize during drag” 只触发 `mouseMove(document)` 后留下注释，没有断言 `onResize` / `onResizeEnd`；Vue spec 甚至只确认 handle 存在和 disabled class。真实拖动回调、document mouseup 清理、键盘 resize 都没有双端覆盖。

**公共内容决策**：复用 core `calculateResizeDelta` / `clampDimensions`，补一个 shared keyboard resize helper 或在双端用同一按键规则；handles 应有稳定 role/focus/ARIA，并与 disabled 状态一致。

**建议修复顺序**：P2。先补 React/Vue 鼠标 move/up 回调断言和 keyboard spec，再实现键盘交互与 ARIA。

**目标验证命令**：

```bash
pnpm vitest run tests/core/resizable-utils.spec.ts tests/react/Resizable.spec.tsx tests/vue/Resizable.spec.ts
pnpm api:validate
pnpm types:check
```
...
#### C32-3 Vue `useDrag` options 是创建期快照，和 React 更新语义不同 — **P3**

**发现问题**

- 🟢 P3｜React [useDrag.ts](../packages/react/src/hooks/useDrag.ts) 使用 `useMemo` / `useCallback` 以 `options.config`、callbacks、`containerId` 为依赖，调用方 rerender 后可更新 disabled、handleSelector、回调和容器 id。
- 🟢 P3｜Vue [useDrag.ts](../packages/vue/src/composables/useDrag.ts) 在 composable 创建时执行 `const config = resolveDragConfig(options.config)`、`const containerId = options.containerId ?? 'default'`、`const callbacks = {...}`；如果调用方传入 reactive config 或后续替换 callback，返回的 action 和 attr helper 不会感知更新。
- 🟢 P3｜现有 Vue spec 只覆盖创建时 config/callback/containerId，React spec 也没有跨 rerender 对照；公开文档没有说明 Vue options 是静态快照还是响应式输入。

**公共内容决策**：两端应明确同一契约。建议 Vue 支持 `MaybeRef` / getter 或在每次 action 时解析最新 options；若保持静态，应在 generated references 和 examples 标明 options 创建后不再追踪。

**建议修复顺序**：P3。先补 Vue reactive config/callback 测试以锁定期望，再决定实现动态追踪或文档化静态契约。

**目标验证命令**：

```bash
pnpm vitest run tests/core/drag.spec.ts tests/react/useDrag.spec.tsx tests/vue/useDrag.spec.ts
pnpm api:validate
pnpm types:check
```
...
#### C32-4 generated references 对 Drag/useDrag 与 C32 高级能力覆盖不足 — **P3**

**发现问题**

- 🟢 P3｜generated [component-index.md](../skills/tigercat/references/component-index.md) 把 `Drag` 路由到 Advanced / `drag.ts`，但 [advanced props](../skills/tigercat/references/shared/props/advanced.md) 没有 `Drag` / `useDrag` 段，[advanced examples](../skills/tigercat/references/examples/advanced.md) 的标准用法列表也没有 Drag/useDrag。
- 🟢 P3｜实际 examples 已有 React/Vue `UseDragDemo`，app-config 中也有 `/use-drag` 路由；generated docs 没有把这个公开 hook/composable 暴露给 skill 使用者，导致 component-index 可发现但 props/examples 不可继续导航。
- 🟢 P3｜generated [layout props](../skills/tigercat/references/shared/props/layout.md) 对 Carousel/Resizable/Splitter 均只列 3 个 props：Carousel 3/13、Resizable 3/12、Splitter 3/8。C32 的关键能力如 Carousel `effect/arrows/infinite/pauseOnHover`、Resizable `handles/axis/lockAspectRatio`、Splitter `gutterSize/disabled/max` 没有进入 compact props。

**公共内容决策**：通过 [generate-api-docs.mjs](../scripts/generate-api-docs.mjs) 或其数据源增强 Advanced/Layout refs；不要手改 generated references。Drag/useDrag 应以 hook/composable 形式进入 examples 和 props/usage note，而不是伪装成普通 `<Drag />` 组件。

**建议修复顺序**：P3。等 C32-1/C32-3 契约落定后补生成器，避免文档继续放大未定语义。
...
#### C32-5 Carousel、Drag、Splitter 基础交互基线当前健康 — **P3 / 观察**

**发现问题**

- ℹ️ Carousel core autoplay controller 覆盖 requestAnimationFrame、stop、visibilitychange；双端组件覆盖 dots/arrows、touch swipe、vertical gesture ignore、passive touch listeners、autoplay pause/resume、imperative API 和 a11y。
- ℹ️ Drag/useDrag core 覆盖 start/over/drop/end、reorder、cross-container move、handleSelector、document drag session cleanup；React/Vue hook/composable 覆盖基础生命周期、drop zone attrs、disabled、dragClass 和容器 id。
- ℹ️ Splitter core 覆盖 size parsing、initial size、clamp、resizePanes、percentage conversion；双端组件覆盖 separator role、keyboard arrow resize、disabled、dragging class、mouse resize start 和 a11y。

**公共内容决策**：这些健康面不需要重构。后续修复 C32-1/C32-2/C32-3 时，应保持当前 Carousel autoplay/touch 行为、Drag 基础生命周期、Splitter keyboard resize 与 shared document drag session cleanup 不回退。

**建议修复顺序**：观察项。作为后续交互能力修复的回归基线。

**目标验证命令**：

```bash
pnpm vitest run tests/core/drag.spec.ts tests/react/useDrag.spec.tsx tests/vue/useDrag.spec.ts tests/core/carousel-utils.spec.ts tests/react/Carousel.spec.tsx tests/vue/Carousel.spec.ts tests/core/splitter-utils.spec.ts tests/react/Splitter.spec.tsx tests/vue/Splitter.spec.ts
```

---

#### C32 公共拆分/合并决策汇总（供任务 H 汇总）

| 项                                       | 决策                                                                       | 优先级 |
| ---------------------------------------- | -------------------------------------------------------------------------- | ------ |
| Splitter sizes 语义（C32-1）             | 按 core 文档收敛为像素语义；如需比例输入应新增明确 API                     | **P2** |
| Resizable keyboard/ARIA（C32-2）         | 补 shared keyboard resize 契约，双端 handles 具备 focus 与 ARIA            | **P2** |
| Vue useDrag options（C32-3）             | 与 React 对齐动态更新，或明确 Vue options 为静态创建期契约                 | P3     |
| generated refs 覆盖（C32-4）             | 由生成器补 Drag/useDrag 入口和 C32 关键 props，不手改 generated references | P3     |
| Carousel/Drag/Splitter 健康基线（C32-5） | 保持现状，作为后续交互能力修复回归基线                                     | 观察   |
...
| 取证                                                                                                 | 结果                                                                                                                            | 对应发现 |
| ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------- |
| grep `SplitterProps.sizes` / `Convert proportional sizes` / Vue `paneSizes.value = [...props.sizes]` | 文档声明像素；React 在可测容器内按比例归一；Vue 直接按像素使用                                                                  | C32-1    |
| grep Resizable handles / `onMouseDown` / `onMousedown` / Resizable specs                             | handles 是仅鼠标 div；缺 role/tabIndex/keyboard；React resize move 测试无断言，Vue 缺真实拖动断言                               | C32-2    |
| grep React/Vue `useDrag` options/callbacks/containerId                                               | React 依赖 options rerender 更新；Vue 创建期解析为快照；测试未覆盖 reactive 更新                                                | C32-3    |
| generated advanced/layout props/examples and `UseDragDemo`                                           | component-index 有 Drag，advanced props/examples 无 Drag/useDrag；layout props 只列 C32 每组件 3 项                             | C32-4    |
