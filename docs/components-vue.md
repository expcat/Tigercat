...
- ChartLegend：Props: items(图例项)/position(位置)/interactive(可交互)；Events: @item-click/@item-hover/@item-leave
- ChartTooltip：Props: content(内容)/visible(可见)/x/y(位置)

#### 高级图表

- BarChart：Props: data(数据)/hoverable(悬停)/selectable(选择)/showLegend(图例)/showTooltip(提示)；Events: @bar-click/@bar-hover
- ScatterChart：Props: data(数据)/pointSize(点大小)/showGrid(网格)
- PieChart：Props: data(数据)/innerRadius(内半径)/hoverable/selectable/showLegend；Events: @slice-click/@slice-hover
...
- LineChart：Props: data/series(数据)/curve(曲线类型)/showPoints(数据点)/hoverable/showLegend；Events: @series-click/@point-hover
- AreaChart：Props: data/series(数据)/stacked(堆叠)/fillOpacity(填充透明度)/hoverable/showLegend；Events: @series-click/@point-hover
