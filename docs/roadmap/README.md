...
| P1     | 剩余 demo 页面批量迁移  | 所有 Vue/React demo 页面                                                                                                     | 所有有 state/handler/mock data 的 DemoBlock 补齐 `script` 属性                                                                                                                        | ⏳ 下一步 |
| P1     | Example 导航与覆盖检查  | `examples/example/shared/app-config.ts`、Vue/React router、demo pages                                                       | 组件导航分组与当前公开组件一致；每个已导出的组件都有对应 demo 页面或明确记录为无需独立 demo；Vue/React demo 覆盖项尽量对齐                                                            | 待开始 |
| P1     | 组件分组文档审查        | Basic、Form、Feedback、Layout、Navigation、Data、Charts、Advanced、Composite                                                | 对每个分组逐项比对共享 Props、Vue 示例、React 示例与源码导出；补齐新增/遗漏 props、默认值、事件、slots/render props、边界状态和主题/i18n 说明                                         | 待开始 |
| P1     | 组件分组 example 审查   | `examples/example/vue3/src/pages`、`examples/example/react/src/pages`                                                       | 每个组件 demo 与当前 API 一致；示例覆盖基础用法、受控/非受控或 v-model、禁用/加载/空态、尺寸/变体、关键交互；移除过时 prop、错误事件名和已废弃模式                                    | 待开始 |
...
3. 批量迁移剩余 demo 页面，并同步清理所有 `mixed` 命名、文案和测试期望。 ⏳ **下一步**
