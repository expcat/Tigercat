...
| P1     | Example 导航与覆盖检查  | `examples/example/shared/app-config.ts`、Vue/React router、demo pages                                                       | 组件导航分组与当前公开组件一致；每个已导出的组件都有对应 demo 页面或明确记录为无需独立 demo；Vue/React demo 覆盖项尽量对齐                                                            | ⏳ 下一步 |
...
4. 按 Basic -> Form -> Feedback -> Layout -> Navigation -> Data -> Charts -> Advanced -> Composite 的顺序审查文档和 example。 ⏳ **下一步**
