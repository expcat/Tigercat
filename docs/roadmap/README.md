...
| P2     | 文档与 example 体验优化 | skills 文档、demo 页面文案与布局                                                                                            | 说明文字更聚焦真实使用场景；代码片段可直接复制；复杂组件补充更接近生产场景的数据；移动端和深色模式下 demo 不出现明显布局溢出                                                          | ✅ 已完成 |
| P2     | 校验与回归              | build、lint、test、examples build                                                                                           | 完成每组审查后运行相关类型检查/测试；Example 模式改造完成后至少构建 Vue 和 React example，必要时补充 DemoBlock 相关单测或 e2e 检查                                                    | ✅ 已完成 |
...
7. ~~P2 文档与 example 体验优化 + 校验与回归。~~ ✅

**所有规划任务已完成。** 后续如有新的优化需求，可在此文档中新增条目。
