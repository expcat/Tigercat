...
- 更新时间：2026-01-14
- 上一步：✅ `List`/`Table` 的 loading overlay spinner classes 统一下沉到 core `loading-utils`（两端删除重复字面量；tests + build 通过）
- 当前组件：Phase 0 基建
...
- [ ] consistent classes：组件 class 生成尽量走 core 的 `*-utils.ts` / `*-styles.ts`
  ...
  - 进度：✅ List/Table 的 loading overlay spinner classes（`animate-spin` + `h-8 w-8` + primary color）已统一下沉到 `@tigercat/core`（`loading-utils`），Vue/React 双端复用同一实现。
  ...
  - 进度：✅ DatePicker/TimePicker 的 20x20 实心 icons paths（`closeSolidIcon20PathD` + calendar/clock/chevron）与 viewBox（`icon20ViewBox`）已统一收敛到 `@tigercat/core`（`common-icons`）；`datepicker-icons`/`timepicker-icons` 复用该常量并保持原导出名不变。
...
- 2026-01-14：Phase 0 基建（consistent classes）：`List`/`Table`（Vue+React）loading overlay spinner class 统一收敛到 core `loading-utils`；相关单测通过；build 通过。
