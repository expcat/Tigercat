...
- 149 个公开组件均已有有效展示入口；R31 已补齐 4 个高优先级组件后，剩余 14 个组件建议扩展功能展示，其中高优先级 6 个、中优先级 8 个。
...
- 当前可执行任务：R32；完成后依次执行 R33-R35，并回写本节和对应批次状态。
...
| R32 | 待执行 | ImageAnnotation、InfiniteScroll、MarkdownEditor    | 受控工具/选区与只读；反向或横向加载；edit/preview 模式、自定义 renderer/toolbar                          |
