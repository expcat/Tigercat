...
**状态**：已完成（2026-06-28）。本轮采用文档先行模式，只更新本节分类、
升级条件与验收边界；未恢复已删除的 `docs/ROADMAP_CHECK.md`，旧扫描明细仅作为
git 历史证据源保留。T13 完成含义是暂缓项已归类并建立后续处理护栏，不代表下表
“低风险清理候选”已经执行源码修复。
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| 分类/文档护栏     | B-5、A-2、C29-3、F-4、C07-5、C01-5                                                                                                                                                                                                   | 补充分类说明、coverage map 或后续 backlog，避免低优先观察在后续任务中被误当成立即修复项；本轮只确认分类，不直接修复。        | 分类文档、只读审计记录、benchmark coverage map、命名/语义说明；guard test 需另开后续任务。 | 不删除公开 API；不改组件运行时；不改 generated references、release gate、locale rollout。 | 审计发现真实功能缺陷、用户可见回归、公共契约不一致，或需要改源码才能建立护栏时，拆成新的 Txx 子任务。       | `corepack pnpm prettier --check docs/ROADMAP.md`、`git diff --check -- docs/ROADMAP.md`。                       |
| 低风险清理候选    | A-3、B-6、C01-1/2/3/6/7、C03-2、C04-5/6/7/8、C05-5、C07-1/3/4/6、C08-2/3/4/5/6/7、C09-2/5/7、C10-3/4/6/7、C11-4/5、C12-4/5/6、C13-4、C14-7、C15-3/4、C17-2、C19-4/5、C20-4/5、C21-6、C22-3、C25-5、C27-5、C28-4、C29-4、C30-4、C32-3 | 保留为未来小批次 backlog，每次只认领一个组件族，处理不急迫的重复实现、helper 归属、轻微 parity、文档发现性或低概率边界问题。 | 小范围 helper 合并、局部测试补强、JSDoc/说明修正、低风险行为对齐；本轮不执行。             | 不跨组件大重构；不把 P2/P1 行为修复混入；不借机收窄公开 prop。                            | 子项需要公共类型变更、破坏兼容、涉及安全/SSR/a11y 主路径，或与 T06/T07/T08/T09/T14 重叠时，升级并重新归类。 | 目标组件/core spec；`corepack pnpm api:validate`、`corepack pnpm types:check`；文档项追加 prettier/diff check。 |
| 观察/健康，无动作 | A-1、C01-8、C07-7、C11-6、C14-8、C16-8、C17-3、C18-8、C19-6、C24-5、C32-5、F-5、G-5                                                                                                                                                  | 保留为后续任务验收的回归背景，不作为实施任务；执行相邻组件修复时用来确认健康路径不被误改。                                   | 只读复核、回归说明、测试备注；必要时把健康路径加入受影响任务的验收清单。                   | 不主动改源码；不把“健康面”改写成待修复缺陷。                                              | 后续取证证明观察项已变成真实缺陷，或产品策略决定改变既有行为时，单独建任务。                                | 文档检查；若相邻源码任务引用该健康面，则跑该任务自己的目标 spec。                                               |

**允许修改**：分类文档、只读审计记录、coverage map、后续 backlog 条目；本轮不新增
guard test，不改任何可执行或生成产物。

**不得修改**：公开 API/类型、package exports/scripts、组件运行时、组件大重构、
generated references、API baseline、changeset、release gate、locale rollout。

**依赖/阻塞**：原则上排在 T01-T12 后；因本轮只落文档分类，且不触碰
T11/T12 的测试维护、CLI/scripts、benchmark 或组件源码文件族，允许先行标记 T13
分类完成。若后续要执行 guard test、coverage map 文件或低风险源码清理，必须重新按
对应文件族确认前置任务与冲突边界。

**公共接口影响**：无公共 API、类型、package exports、package scripts、generated
references、API baseline 或 changeset 变更。

**完成验证**：

- `corepack pnpm prettier --check docs/ROADMAP.md`
- `git diff --check -- docs/ROADMAP.md`
- `git diff --name-only` 仅包含 `docs/ROADMAP.md`
