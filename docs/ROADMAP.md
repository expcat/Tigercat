...
**完成摘要**：已覆盖 React/Vue Charts 与 Advanced 共 48 个页面，清理旧 snippet 常量，并把每页重复 `DemoBlock` 合并为带场景小标题的单块展示；CodeEditor、MarkdownEditor、FileManager、VirtualTable 增加可编译 fixture source 作为 Code 来源。详细执行记录见 [V2_COMPLETED.md](V2_COMPLETED.md#r26-charts-and-advanced-demo-merging)。

**完成验证**：`npx -y pnpm@11.9.0 test:group:charts`、`npx -y pnpm@11.9.0 test:group:advanced`、`npx -y pnpm@11.9.0 example:sources:check`、`npx -y pnpm@11.9.0 example:build` 已通过；完整门禁记录见 [V2_COMPLETED.md](V2_COMPLETED.md#r26-charts-and-advanced-demo-merging)。
