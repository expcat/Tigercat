...
| 组件       | 增强内容                                                                               | 优先级 | 状态                                                             |
| ---------- | -------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------- |
| **Table**  | 列拖拽排序、行拖拽排序、列固定增强、单元格编辑、导出 Excel/CSV 增强                    | 高     | 已完成：列/行拖拽、列固定、单元格编辑、CSV/Excel 导出            |
...
| **Tree**   | 大数据虚拟化（>10000 节点）、节点拖拽排序、搜索高亮                                    | 中     | 已完成：虚拟化、节点拖拽、搜索高亮                               |
| **Upload** | 断点续传、分片上传、上传队列管理                                                       | 中     | 已完成：分片上传、续传元数据、队列并发控制                       |
| **Menu**   | 菜单搜索过滤、折叠模式增强                                                             | 中     | 已完成：菜单搜索过滤、折叠模式                                   |
| **Charts** | 数据缩放（brush）、导出 PNG/SVG、图表联动、自适应 resize 增强                          | 中     | 已完成：brush 工具、PNG/SVG 导出、联动 controller、自适应 resize |
| **Tour**   | 步骤条件跳过、异步步骤加载、移动端适配                                                 | 低     | 已完成：条件跳过、异步步骤加载、移动端定位约束                   |
| **Kanban** | 泳道分组、WIP 限制可视化、卡片过滤                                                     | 低     | 已完成：泳道、WIP 可视化、卡片过滤                               |

### 7.2 性能优化

#### Bundle 优化

- [x] Tree-shaking 审计：确保子路径导入 `@expcat/tigercat-vue/Button` 可用
- [x] 重型组件懒加载：Modal、Drawer、DatePicker、TimePicker、ColorPicker、Charts
- [x] Chart 按需加载：每种图表独立 entry（`@expcat/tigercat-vue/LineChart`）
- [x] size-limit 细化：新增代表性子路径 size 检查（Button < 15KB gzip）

#### 运行时优化

- [x] 虚拟滚动统一：VirtualList / VirtualTable 共享滚动引擎，减少重复实现
- [x] 大数据 Table：>10000 行时自动启用虚拟滚动
- [x] Form 性能：大表单场景（>50 字段）校验防抖、按需校验（仅校验变更字段）
  - [x] 按需校验（仅校验变更字段）
  - [x] 校验防抖
- [x] 事件委托：Table 行点击/行拖拽改用 tbody 级委托，Menu / Tree 保留必要控件级监听避免交互回归
- [x] CSS 变量缓存：主题切换时批量更新，避免重复 setProperty
