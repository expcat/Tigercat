...
- **i18n** 空态 / 终态文案收敛补全（接 B / C 复审 I18N-1 / I18N-2）：`TigerLocaleCommon` 新增 `noMoreText`（14 种语言预设已补齐）。双端 `Select`（`noOptionsText` / `noDataText` → `common.emptyText`）、`FileManager`（`emptyText` → `common.emptyText`）、`InfiniteScroll`（`endText` → `common.noMoreText`）的英文默认值下沉为 `resolveLocaleText` 回退首参，未传时回退 `mergeTigerLocale(ConfigProvider locale, props.locale)`；并修复 Vue `Cascader` 空态此前直接渲染 `notFoundText`、未接 locale 的漏改（React 端早已正确），使其与本节既有「双端 `Cascader` `notFoundText` 回退 `common.*`」承诺一致。显式 prop 仍为最高优先级，未配置 ConfigProvider 时英文渲染不变。
