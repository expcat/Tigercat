...
- 统一 React **ImagePreview** 门户的 SSR 守卫：`createPortal(..., document.body)` 改用 `utils/overlay.ts` 的 `renderBodyPortal`（内含 `isBrowser()`），与 Tour / FloatButton / ChartTooltip 等门户组件保持一致，不再隐式依赖 `isOpen` 状态兜底；无行为变更。配套将测试基建 `tests/setup.ts` 的 `matchMedia` mock 加 `typeof window` 守卫，使 node 环境（SSR）spec 可共用同一 setup。
- 下沉跨端重复的命令式 API 与 InputNumber helper 到 core（无行为变更）：core `imperative-api` 新增 `createInstanceIdGenerator`（独立自增计数器工厂），Message / Notification 双端各自内联的 `getNextInstanceId` + `instanceIdCounter` 改用之（每处各建独立计数器，保持 Message 与 Notification 的 id 序列互不影响）；core `input-number-utils` 新增 `formatInputNumberDisplay` / `parseInputNumberInput`，双端 **InputNumber** 的 `toDisplayValue` / `parseValue` 改调同一份 core 实现。Message / Notification 的 `normalizeOptions` 仍为对 core `normalizeStringOption` 的薄包装（非重复实现），保持不变。
...
- 修复手维护指南两处时效问题：`skills/tigercat/references/cli.md` 命令表内未转义的 `|`（`--template vue3 | react`、`--framework vue3 | react | both`）此前撑坏表格列与表头分隔行，改为 `vue3 / react` 文字表述并还原表头为 3 列；`skills/tigercat/references/performance.md` 补「Runtime Benchmarks」一节，说明 `pnpm bench`（`vitest bench`）、`benchmarks/` 下 8 个 `.bench.ts` 与 `vitest.config.ts` benchmark 段，并注明其为手工性能回归工具（尚未接入 CI、无基线阈值）。
