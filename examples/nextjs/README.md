...
`app/globals.css` imports Tailwind, `@plugin '@expcat/tigercat-core/tailwind'`, and `@source` on `packages/react/dist` and `packages/core/dist` (Next's PostCSS scan does not follow the pnpm `node_modules` symlink). `layout.tsx` sets `<html lang="zh-CN">` and wraps `children` with `ConfigProvider locale={zhCN} colorScheme="light"`.
