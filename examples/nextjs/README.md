...
`app/globals.css` imports Tailwind, `@plugin '@expcat/tigercat-core/tailwind'`, and `@source` on the React/core `dist` files. `layout.tsx` sets `<html lang="zh-CN">` and wraps `children` with `ConfigProvider locale={zhCN} colorScheme="light"`.
