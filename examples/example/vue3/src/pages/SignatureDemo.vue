...
<template>
  <div class="max-w-4xl mx-auto p-4 sm:p-8">
    ...
    <DemoBlock title="基本用法" description="通过 change 事件获取当前签名导出值" :code="basicSnippet">
      <div class="space-y-3">
        <Signature
          :width="420"
          :height="180"
    ...
    <DemoBlock title="导出" description="通过 ref 导出 PNG data URL 或 SVG 字符串" :code="exportSnippet">
      <div class="space-y-4">
        ...
        <pre
          v-if="svgText"
          class="max-h-40 overflow-auto rounded border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">{{ svgText }}</pre>
      </div>
    </DemoBlock>

    <DemoBlock title="禁用和只读" description="disabled / readonly 状态下不可绘制" :code="disabledSnippet">
