...
const placementSnippet = `<div class="grid grid-cols-4 gap-4">
...
<template>
  <div class="max-w-5xl mx-auto p-8">
    ...
    <!-- 触发方式 -->
    <DemoBlock title="触发方式" description="支持 hover（默认）、click、focus 三种触发方式。" :code="triggerSnippet">
      <Space :size="16">
        <Tooltip content="悬停触发（默认）" trigger="hover">
          <Button>Hover</Button>
        </Tooltip>
    ...
    <!-- 受控模式 -->
    <DemoBlock title="受控模式" description="可以通过 visible 属性控制气泡提示的显示和隐藏。" :code="controlledSnippet">
      <Space :size="16">
        <Tooltip v-model:visible="visible1" content="受控的气泡提示">
          <Button>受控提示</Button>
        </Tooltip>
