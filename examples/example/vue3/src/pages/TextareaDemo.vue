...
const rowsSnippet = `<Space direction="vertical" class="w-full max-w-md">
  <FormItem label="2行">
...
const autoResizeSnippet = `<Space direction="vertical" class="w-full max-w-md">
...
const countSnippet = `<Space direction="vertical" class="w-full max-w-md">
  <FormItem label="仅计数">
    <Textarea showCount placeholder="显示字符数" />
  </FormItem>
  <FormItem label="限制最大长度 (100)">
    <Textarea v-model="limited" showCount :maxLength="100" placeholder="最多 100 个字符" />
  </FormItem>
</Space>`
...
<template>
  <div class="max-w-5xl mx-auto p-8">
    ...
    <!-- 不同行数 -->
    <DemoBlock title="不同行数"
               description="通过 rows 属性设置文本域的行数。"
               :code="rowsSnippet">
      <Space direction="vertical"
             class="w-full max-w-md">
        <FormItem label="2行">
          <Textarea placeholder="2行文本域"
                    :rows="2" />
        </FormItem>
        <FormItem label="4行">
          <Textarea placeholder="4行文本域"
                    :rows="4" />
        </FormItem>
        <FormItem label="6行">
          <Textarea placeholder="6行文本域"
                    :rows="6" />
        </FormItem>
      </Space>
    </DemoBlock>
    ...
    <DemoBlock title="自动高度"
      ...
      <Space direction="vertical"
             class="w-full max-w-md">
        <FormItem label="基础自动高度">
          <Textarea v-model="autoResizeText"
                    autoResize
    ...
    <DemoBlock title="禁用和只读"
      ...
      <Space direction="vertical"
             class="w-full max-w-md">
        <Textarea v-model="disabled"
                  disabled
                  :rows="3" />
        <Textarea v-model="readonly"
                  readonly
                  :rows="3" />
      </Space>
    </DemoBlock>
