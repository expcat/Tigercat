...
export default function OrgChartDemo() {
  ...
  const selectedNode = selectedId === null ? undefined : findOrgNode(orgData, selectedId)
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-600">
                当前选择：
                {selectedNode
                  ? `${selectedNode.label}${selectedNode.title ? ` / ${selectedNode.title}` : ''}（${String(selectedNode.id)}）`
                  : '未选择'}
              </div>
            </div>
          </section>
