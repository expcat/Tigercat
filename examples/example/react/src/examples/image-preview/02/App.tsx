...
const images = [
  { src: 'https://picsum.photos/seed/tiger-preview-scale-1/800/600', alt: '林间小径' },
  { src: 'https://picsum.photos/seed/tiger-preview-scale-2/800/600', alt: '湖面倒影' },
  { src: 'https://picsum.photos/seed/tiger-preview-scale-3/800/600', alt: '山脊云雾' }
]

export default function App() {
  const [open, setOpen] = useState(false)
  const [currentIndex, setCurrentIndex] = useState(0)
  const [status, setStatus] = useState('选择一张图片打开预览')
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    setOpen(nextOpen)
    if (!nextOpen) {
      setStatus(`预览已从第 ${currentIndex + 1} 张图片关闭`)
    }
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-[var(--tiger-text-secondary)]" aria-live="polite">
