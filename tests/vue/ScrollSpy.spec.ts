...
describe('ScrollSpy', () => {
  ...
  const renderScrollSpy = (props: Record<string, unknown> = {}) =>
    render(ScrollSpy, {
      props: {
        items,
  ...
  describe('Rendering', () => {
    ...
    it('applies attrs class', () => {
      const { container } = render(ScrollSpy, {
        props: { items, getContainer: () => scrollContainer },
        attrs: { class: 'attr-class' }
      })
      expect(container.firstChild).toHaveClass('attr-class')
    })
  ...
  describe('Active State', () => {
    ...
    it('does not change the rendered active item in controlled mode after click', async () => {
      renderScrollSpy({ activeKey: 'intro' })
  ...
  describe('Events', () => {
    it('emits click when an enabled item is clicked', async () => {
      const result = renderScrollSpy()
      await fireEvent.click(screen.getByText('Usage'))
      expect(result.emitted('click')?.[0][0]).toMatchObject({ key: 'usage' })
    })

    it('emits change with payload when active item changes by click', async () => {
      const result = renderScrollSpy()
      await fireEvent.click(screen.getByText('Usage'))
      expect(result.emitted('change')?.[0]).toEqual([
        'usage',
    ...
    it('emits update:activeKey when active item changes', async () => {
      const result = renderScrollSpy()
      await fireEvent.click(screen.getByText('Usage'))
      expect(result.emitted('update:activeKey')?.[0]).toEqual(['usage'])
    })
    ...
    it('scrolls the section container when the nav is outside it', async () => {
      const scrollToSpy = vi.spyOn(scrollContainer, 'scrollTo')
      vi.spyOn(window.history, 'replaceState').mockImplementation(() => {
        throw new DOMException('replaceState', 'SecurityError')
      })
      render(ScrollSpy, { props: { items, orientation: 'horizontal' } })
      await fireEvent.click(screen.getByText('Usage'))
      expect(scrollToSpy).toHaveBeenCalled()
      expect(screen.getByText('Usage')).toHaveAttribute('aria-current', 'location')
    })

    it('scrolls to the clicked target', async () => {
      const scrollToSpy = vi.spyOn(scrollContainer, 'scrollTo')
    ...
    it('does not emit events for disabled items', async () => {
      ...
      expect(result.emitted('click')).toBeUndefined()
    })

    it('does not emit change when clicking the active item', async () => {
      const result = renderScrollSpy()
