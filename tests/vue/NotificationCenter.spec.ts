...
describe('NotificationCenter (Vue)', () => {
  it('renders groups and switches tabs', async () => {
    ...
    render(NotificationCenter, {
      props: { items, groupBy: (item: NotificationItem) => String(item.type ?? '') }
    })
  ...
  it('renders empty state with emptyText', () => {
    render(NotificationCenter, { props: { items: [], emptyText: '暂无通知' } })
    expect(screen.getByText('暂无通知')).toBeInTheDocument()
  })

  it('renders loading state', () => {
    render(NotificationCenter, { props: { loading: true, loadingText: '加载中...' } })
  ...
  it('renders custom title', () => {
    ...
    render(NotificationCenter, { props: { items, title: '消息中心' } })
