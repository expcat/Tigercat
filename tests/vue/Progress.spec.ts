...
const progressVariants = ['default', 'primary', 'success', 'warning', 'danger', 'info'] as const
...
describe('Progress', () => {
  ...
  describe('Variants', () => {
    it.each(progressVariants)('should render %s variant correctly', (variant) => {
      const { container } = renderWithProps(Progress, {
        variant,
        percentage: 50,
      })
  ...
  describe('Sizes', () => {
    it.each(componentSizes)('should render %s size correctly for line', (size) => {
      const { container } = renderWithProps(Progress, {
        size,
        percentage: 50,
        type: 'line',
    ...
    it.each(componentSizes)('should render %s size correctly for circle', (size) => {
      const { container } = renderWithProps(Progress, {
        size,
        percentage: 50,
        type: 'circle',
  ...
  describe('Percentage', () => {
    ...
    it('should clamp percentage to 0-100 range', () => {
      const { container: container1 } = renderWithProps(Progress, { percentage: -10 })
      const progressbar1 = container1.querySelector('[role="progressbar"]')
      expect(progressbar1).toHaveAttribute('aria-valuenow', '0')

      const { container: container2 } = renderWithProps(Progress, { percentage: 150 })
