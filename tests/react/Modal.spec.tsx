...
describe('Modal', () => {
  ...
  describe('Events', () => {
    ...
    it('should restore focus when the open instance is unmounted', async () => {
      ...
      const { unmount } = render(<Modal open={true} title="Test Modal" />)
      ...
      function Harness() {
        ...
        return (
          <>
            ...
            <Modal open={open} onOpenChange={setOpen} title="Focus Trap Test" showDefaultFooter>
  ...
  describe('anchored overlay layer', () => {
    it('owns a layer host and mounts nested anchored overlays into it', async () => {
      render(
        <Modal open={true} title="Layer host">
          <Select options={[{ label: 'Option', value: 'option' }]} />
        </Modal>
      )
      ...
      fireEvent.click(dialog.querySelector('[role="combobox"]')!)
    ...
    it('closes a default-open nested overlay before the modal on Escape', async () => {
      ...
      const onOpenChange = vi.fn()
      render(
        <Modal open={true} title="Parent modal" onOpenChange={onOpenChange}>
          <Popover defaultOpen content="Nested popover">
