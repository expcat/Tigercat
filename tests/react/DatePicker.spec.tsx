...
describe('DatePicker', () => {
  describe('Rendering', () => {
    ...
    it('should display date in different formats', () => {
      ...
      formats.forEach(({ format, expected }) => {
        const { container } = render(<DatePicker value={testDate} format={format} />)
  ...
  describe('States', () => {
    ...
    it('should show/hide clear button based on clearable', () => {
      ...
      expect(withClear.querySelector('button[aria-label="Clear date"]')).toBeInTheDocument()

      const { container: withoutClear } = render(
        <DatePicker value={new Date('2024-01-15')} clearable={false} />
      )
      expect(withoutClear.querySelector('button[aria-label="Clear date"]')).not.toBeInTheDocument()
    })
  })
  ...
  describe('Calendar Interaction', () => {
    ...
    it('should navigate to previous/next month', async () => {
      const user = userEvent.setup()
      const { container } = render(<DatePicker value={new Date('2024-03-15')} />)
    ...
    it('should keep calendar open in range mode until OK is clicked', async () => {
      ...
      const beforeStart = screen.getByLabelText('2024-03-05') as HTMLButtonElement
  ...
  describe('Keyboard', () => {
    it('should move focus with ArrowRight', async () => {
      ...
      expect(screen.getByLabelText('2024-03-16') as HTMLButtonElement).toHaveFocus()
    })
    ...
    it('should navigate across months with ArrowRight', async () => {
      ...
      const endOfMonth = screen.getByLabelText('2024-03-31') as HTMLButtonElement
      endOfMonth.focus()
      ...
      await waitFor(() => {
        expect(screen.getByLabelText('2024-04-01') as HTMLButtonElement).toHaveFocus()
      })
    })
  ...
  describe('Controlled vs Uncontrolled', () => {
    it('should work as controlled component', async () => {
      const user = userEvent.setup()
      const onChange = vi.fn()
      const { container, rerender } = render(<DatePicker value={null} onChange={onChange} />)
  ...
  describe('Date Constraints', () => {
    it('should disable dates before minDate', async () => {
      const user = userEvent.setup()
      const { container } = render(
        <DatePicker value={new Date('2024-01-15')} minDate={new Date('2024-01-10')} />
      )
    ...
    it('should disable dates after maxDate', async () => {
      const user = userEvent.setup()
      const { container } = render(
        <DatePicker value={new Date('2024-01-15')} maxDate={new Date('2024-01-20')} />
      )
