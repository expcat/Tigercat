...
describe('Descriptions (React)', () => {
  it('renders title and extra', () => {
    ...
    render(<Descriptions title="User" items={items} extra={<a href="#">Edit</a>} />)
  ...
  it('respects column and span in horizontal layout', () => {
    ...
    const { container: spanContainer } = render(<Descriptions items={itemsWithSpan} column={3} />)
  ...
  it('passes through div attributes', () => {
    const items: DescriptionsItem[] = [{ label: 'Name', content: 'John Doe' }]
    render(
      <Descriptions items={items} data-testid="root" aria-label="descriptions" className="custom" />
    )
