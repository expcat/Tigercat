...
describe('Upload', () => {
  ...
  describe('Props', () => {
    ...
    it('should hide file list when showFileList is false', () => {
      ...
      const { container } = render(<Upload fileList={fileList} showFileList={false} />)
  ...
  describe('Events', () => {
    ...
    it('should accumulate multiple selected files into fileList', async () => {
      const onChange = vi.fn()
      const { container } = render(<Upload multiple fileList={[]} onChange={onChange} />)
    ...
    it('should call onExceed when file limit is exceeded', async () => {
      ...
      const { container } = render(<Upload fileList={fileList} limit={1} onExceed={onExceed} />)
    ...
    it('should call onPreview when preview button is clicked in picture-card mode', async () => {
      ...
      const { container } = render(
        <Upload fileList={fileList} listType="picture-card" onPreview={onPreview} />
      )
  ...
  describe('File Validation', () => {
    it('should validate file type', async () => {
      const onChange = vi.fn()
      const { container } = render(<Upload accept="image/*" onChange={onChange} />)
  ...
  describe('Drag and Drop', () => {
    ...
    it('should not process dropped files when disabled', async () => {
      const onChange = vi.fn()
      const { container } = render(<Upload drag disabled onChange={onChange} />)
