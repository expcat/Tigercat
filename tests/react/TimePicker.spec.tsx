...
describe('TimePicker', () => {
  describe('Rendering', () => {
    ...
    it('should display time with seconds', () => {
      const { container } = render(<TimePicker value="14:30:45" showSeconds={true} />)
  ...
  describe('Clear Button', () => {
    it('should show clear button when value is present and clearable is true', () => {
      const { container } = render(<TimePicker value="14:30" clearable={true} />)
    ...
    it('should not show clear button when clearable is false', () => {
      const { container } = render(<TimePicker value="14:30" clearable={false} />)
    ...
    it('should call onChange with null when clear button is clicked', async () => {
      ...
      const { container } = render(
        <TimePicker value="14:30" clearable={true} onChange={onChange} onClear={onClear} />
      )
  ...
  describe('Time Panel', () => {
    ...
    it('should display AM/PM column when format is 12', async () => {
      ...
      await waitFor(() => {
        ...
        const hasAM = Array.from(buttons || []).some((el) => el.textContent === 'AM')
        const hasPM = Array.from(buttons || []).some((el) => el.textContent === 'PM')
  ...
  describe('Time Selection', () => {
    it('should call onChange when time is selected', async () => {
      ...
      const panel = container.querySelector('[role="dialog"]')
      const timeButtons = panel?.querySelectorAll('button[aria-label*="hours"]')
    ...
    it('should work in controlled mode', async () => {
      const user = userEvent.setup()
      const onChange = vi.fn()
      const { container, rerender } = render(<TimePicker value="10:00" onChange={onChange} />)
  ...
  describe('Footer Buttons', () => {
    it('should have Now and OK buttons in footer', async () => {
      ...
      await waitFor(() => {
        ...
        const buttons = panel?.querySelectorAll('button')
        const nowButton = Array.from(buttons || []).find((el) => el.textContent === 'Now')
        const okButton = Array.from(buttons || []).find((el) => el.textContent === 'OK')
