...
describe("Notification (React)", () => {
  ...
  it("respects position by creating a container per position", async () => {
    ...
    const container = document.getElementById(
      "tiger-notification-container-top-left"
    );
    expect(container).toBeTruthy();
    expect(container?.getAttribute("data-tiger-notification-position")).toBe(
      "top-left"
    );
  ...
  it("clears notifications for a specific position", async () => {
    ...
    expect(document.querySelectorAll("[data-tiger-notification]").length).toBe(
      2
    );
