...
describe('Affix', () => {
  ...
  it('pins offsetBottom to the custom target container bottom, not the viewport', async () => {
    ...
    render(
      <Affix target="#affix-bottom-root" offsetBottom={8} data-testid="affix-target-bottom">
        Bottom pinned
      </Affix>
    )
    ...
    expect(screen.getByTestId('affix-target-bottom').nextElementSibling).toHaveAttribute(
      'aria-hidden',
      'true'
    )
  ...
  it('does not throw when the target selector is illegal', async () => {
    expect(() => {
      render(
        <Affix target="[" offsetTop={0} data-testid="affix-bad">
          Content
        </Affix>
      )
    }).not.toThrow()
  ...
  it('updates z-index without rebuilding the observer', async () => {
    const { rerender } = render(
      <Affix offsetTop={0} zIndex={10} data-testid="affix-z-live">
        Content
      </Affix>
    )
