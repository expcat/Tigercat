...
describe('ImageAnnotation', () => {
  ...
  it('keeps a clicked shape selected so Delete enables', async () => {
    ...
    const onSelect = vi.fn()
    const { getByRole, getByLabelText } = await renderLoadedAnnotation(
      <ImageAnnotation src="/scene.jpg" defaultValue={annotations} onSelect={onSelect} />
    )
    const shape = getByRole('option', { name: 'Rectangle annotation' })
    ...
    fireEvent.click(getByLabelText(/Image annotation canvas/))
    expect(remove).toBeDisabled()
  ...
  it('draws a freehand stroke that follows the pointer and stays open', async () => {
    const onChange = vi.fn()
    const { getByRole, getByLabelText, container } = await renderLoadedAnnotation(
      <ImageAnnotation src="/scene.jpg" onChange={onChange} />
    )
    ...
    const annotations = onChange.mock.calls.at(-1)?.[0] as CoreImageAnnotation[]
