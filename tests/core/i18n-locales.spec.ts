...
describe('i18n locale presets', () => {
  ...
  it('all locales have select.doneText, placeholder, emptyText, and createOptionLabel', () => {
    for (const [, locale] of Object.entries(locales)) {
      ...
      expect(locale.select).not.toHaveProperty('levelLabel')
      expect(locale.select).not.toHaveProperty('backText')
      expect(locale.select).not.toHaveProperty('expandAriaLabel')
      expect(locale.select).not.toHaveProperty('collapseAriaLabel')
      expect(locale.cascader?.levelLabel).toBeDefined()
      expect(locale.cascader?.backText).toBeDefined()
      expect(locale.cascader?.expandAriaLabel).toBeDefined()
      expect(locale.cascader?.collapseAriaLabel).toBeDefined()
      expect(locale.treeSelect?.levelLabel).toBeDefined()
      expect(locale.treeSelect?.backText).toBeDefined()
      expect(locale.treeSelect?.expandAriaLabel).toBeDefined()
      expect(locale.treeSelect?.collapseAriaLabel).toBeDefined()
    }
  })
  ...
  it('workflow timeline and designer packs include 2.4.2 scanability keys', () => {
    ...
    expect(zhCN.workflowTimeline?.returnTarget).toBe('退回目标')
    expect(zhCN.workflowTimeline?.signOrsignAny).toBe('任一人')
    expect(enUS.workflowTimeline?.returnTarget).toBe('Returned here')
    expect(enUS.workflowTimeline?.signOrsignAny).toBe('Any one')
