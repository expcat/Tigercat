...
describe('overlay-utils (core)', () => {
  ...
  it('isEventOutside should treat ignored elements as inside', () => {
    ...
    expect(isEventOutside(event, [container], { ignore: [trigger] })).toBe(
      false
    );
  });
  ...
  it('getFocusTrapNavigation should wrap focus on Tab at edges', () => {
    ...
    expect(
      getFocusTrapNavigation({ key: 'Tab', shiftKey: false }, focusables, b)
    ).toEqual({ shouldHandle: true, next: a });

    expect(
      getFocusTrapNavigation({ key: 'Tab', shiftKey: true }, focusables, a)
    ).toEqual({ shouldHandle: true, next: b });
  });
});
