...
- `Header` blur/translucent variants gain overridable tuning tokens: `--tiger-header-saturate` (adds `backdrop-saturate`) and `--tiger-header-shadow` (drives the `blur` variant's shadow), so custom shadow/saturation can be applied via low-specificity tokens without `!important`.
...
- `Icon` built-in set expanded with common app glyphs: `home`, `bell`, `mail`, `phone`, `download`, `upload`, `filter`, `refresh`, `logout`, `lock`, `star`, `heart`, `copy`, `link`, `document`, `folder`, `image`, `map-pin`, `check-circle`, `x-circle`.
