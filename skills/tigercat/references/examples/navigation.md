...
标准用法 `<Component />`（Vue/React 同名，绑定差异见 `shared/patterns/common.md`）：Affix, Anchor, BackTop, Breadcrumb, BreadcrumbItem, FloatButton, FloatButtonGroup, NavigationMenu, NavigationMenuContent, NavigationMenuItem, NavigationMenuLink, NavigationMenuTrigger, PageHeader, Pagination, ScrollSpy, Spotlight, Steps, Tabs.
