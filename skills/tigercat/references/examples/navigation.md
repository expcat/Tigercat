...
标准用法 `<Component />`（Vue/React 同名，绑定差异见 `shared/patterns/common.md`）：Affix, Anchor, AnchorLink, BackTop, Breadcrumb, BreadcrumbItem, ContextMenu, ContextMenuItem, ContextMenuMenu, ContextMenuSub, Dropdown, DropdownItem, DropdownMenu, FloatButton, FloatButtonGroup, Menu, MenuItem, MenuItemGroup, Pagination, ScrollSpy, Spotlight, Steps, StepsItem, SubMenu, TabPane, Tabs, Tree.
