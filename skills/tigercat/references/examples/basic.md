...
标准用法 `<Component />`（Vue/React 同名，绑定差异见 `shared/patterns/common.md`）：Alert, Avatar, AvatarGroup, Badge, Button, ButtonGroup, Code, ConfigProvider, CropUpload, Divider, Empty, Highlight, Image, ImageCropper, ImageGroup, ImagePreview, Kbd, Link, QRCode, Rate, Result, Segmented, Statistic, Tag, Text, Watermark.
