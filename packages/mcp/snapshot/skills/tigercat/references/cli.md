...
| -------------------------------- | ----------------------------- | ----------------------------------------------------- |
| `tigercat create <name>`         | Create a Vue 3 or React app   | `--template vue3 \| react`, `--preset blank \| shell` |
| `tigercat migrate`               | Rewrite deleted import names  | `--write`, `--dir`                                    |
...
CLI templates and `doctor` are Tailwind CSS v4-only. `doctor` fails missing, old, or unverifiable Tailwind and requires `@tailwindcss/vite` v4. The CLI version is the CLI `package.json` version. `doctor` requires that same major. Component names come from `scripts/lib/public-components.mjs`. API docs are `pnpm docs:api`, not a CLI command. `add` writes subpath imports and installs the locked package version. When both Vue and React are installed, pass `--framework`. `create --preset shell` scaffolds `AppShell` with a static route-title table. `create` lists files before overwrite. `migrate` previews import-specifier and deleted-name rewrites (`Kanban`, `ImageViewer`, `DonutChart`, `Stepper`); `--write` updates the listed files and does not rewrite props. The name list is `REMOVED_PUBLIC_NAMES` in `scripts/lib/public-components.mjs`. `doctor` fails when the packaged skill catalog version or file digest disagrees with this CLI. Templates write `.gitignore` and `packageManager`. Snippet and generate output stay inside the project, and existing files are skipped. An existing playground refreshes its lockfile from the current template before `pnpm exec vite`.
