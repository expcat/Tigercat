...
export const Slider: React.FC<SliderProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent, value: number, thumbType: 'min' | 'max' | null) => {
    if (disabled) return
  ...
  const rangeStyles: React.CSSProperties = range && Array.isArray(internalValue)
    ? {
    ...
    : {
        left: '0%',
        width: `${getPercentage(typeof internalValue === 'number' ? internalValue : internalValue[0])}%`,
      }
  ...
  return (
    <div className={classNames(sliderBaseClasses, disabled && 'cursor-not-allowed', className)} {...props}>
      {/* Track */}
