...
export const Select = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const flatFilteredOptions = computed(() =>
      flattenOptions(filteredOptions.value)
    );
    ...
    const displayText = computed(() => {
      if (props.multiple && Array.isArray(props.modelValue)) {
        ...
        const selectedOptions = allOptions.value.filter((opt) =>
    ...
    const findNextEnabledIndex = (
      ...
      for (
        let i = start;
        i >= 0 && i < flatFilteredOptions.value.length;
        i += direction
      ) {
        if (!flatFilteredOptions.value[i]?.disabled) {
          return i;
    ...
    function handleSearchKeyDown(event: KeyboardEvent) {
      switch (event.key) {
        ...
        case "ArrowUp": {
          ...
          event.stopPropagation();
          const next =
            activeIndex.value >= 0 ? activeIndex.value : findLastEnabledIndex();
          setActiveAndFocus(next);
          return;
        }
        case "Enter": {
