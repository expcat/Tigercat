...
export const FormItem = defineComponent({
  ...
  setup(props, { slots }) {
    const formContextRef = inject<ComputedRef<FormContext> | null>(
      FormContextKey,
      null
    );
    ...
    return () => {
      ...
      const fieldChildren = (() => {
        ...
        const existingOnFocusout = existingProps.onFocusout as
          | ((event: FocusEvent) => void)
          | undefined;
        const existingOnInput = existingProps.onInput as
          | ((event: Event) => void)
          | undefined;
      ...
      const labelElement = props.label
        ? h(
            ...
            [
              isRequired.value &&
                h("span", { class: "tiger-form-item__asterisk" }, "*"),
              props.label,
            ]
          )
        : null;
