import { defineComponent, computed, inject, ref, h, PropType, Transition } from 'vue'
import {
  classNames,
...
export const SubMenu = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const indentStyle = computed(() => {
      if (!menuContext || menuContext.mode === 'horizontal' || props.level === 0) {
        return {}
      }
    ...
    return () => {
      ...
      if (props.icon) {
        if (typeof props.icon === 'string') {
          ...
          titleChildren.push(
            h('span', { class: menuItemIconClasses }, props.icon)
          )
        }
      }
      ...
      const contentNode = menuContext.mode === 'horizontal'
        ? h(
            'ul',
            {
