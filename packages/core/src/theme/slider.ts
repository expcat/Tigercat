 ...
 */
export const sliderTrackClasses = 'bg-gray-200 rounded-full'
 ...
 */
export const sliderThumbClasses = 'bg-white border-2 border-[var(--tiger-primary,#2563eb)] rounded-full absolute top-1/2 -translate-y-1/2 cursor-pointer transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--tiger-primary,#2563eb)]'
 ...
 */
export const sliderTooltipClasses = 'absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white rounded whitespace-nowrap pointer-events-none'
...
export function getSliderTrackClasses(
  ...
  return [sliderTrackClasses, sizeClass, disabledClass]
    .filter(Boolean)
    .join(' ')
}
...
export function getSliderThumbClasses(
  ...
  return [sliderThumbClasses, sizeClass, disabledClass]
    .filter(Boolean)
    .join(' ')
}
 ...
 */
export function getSliderTooltipClasses(
  size: SliderSize = 'md'
): string {
  const sizeClass = sliderSizeClasses[size].tooltip
  
  return [sliderTooltipClasses, sizeClass]
    .filter(Boolean)
    .join(' ')
}
