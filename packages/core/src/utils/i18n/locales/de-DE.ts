/**
 * German (Germany) (de-DE).
 */

import type { TigerLocale } from '../../../types/locale'
import { DE_DE_DATEPICKER_LOCALE } from '../datepicker-locales/de-DE'

export const deDE: TigerLocale = {
  locale: 'de-DE',
  direction: 'ltr',
  datePicker: DE_DE_DATEPICKER_LOCALE,
  common: {
    okText: 'OK',
    cancelText: 'Abbrechen',
    closeText: 'Schließen',
    loadingText: 'Wird geladen...',
    loadingFinishedText: 'Laden abgeschlossen',
    loadingFailedText: 'Laden fehlgeschlagen',
    viewText: 'Anzeigen',
    emptyText: 'Keine Daten',
    noMoreText: 'Keine weiteren Daten',
    searchPlaceholder: 'Suchen',
    clearText: 'Löschen',
    closeMessageAriaLabel: 'Nachricht schließen',
    closeNotificationAriaLabel: 'Benachrichtigung schließen',
    sidebarAriaLabel: 'Seitenleiste',
    moreOptionsText: 'Weitere Optionen',
    confirmTitle: 'Möchten Sie fortfahren?'
  },
  empty: {
    noData: 'Keine Daten',
    noDataAvailable: 'Keine Daten verfügbar',
    noResults: 'Keine Ergebnisse',
    error: 'Etwas ist schiefgelaufen'
  },
  modal: {
    closeAriaLabel: 'Schließen',
    okText: 'OK',
    cancelText: 'Abbrechen',
    dialogAriaLabel: 'Dialogfenster'
  },
  drawer: {
    closeAriaLabel: 'Schließen',
    dialogAriaLabel: 'Schublade'
  },
  alert: {
    closeAriaLabel: 'Warnung schließen'
  },
  qrcode: {
    ariaLabel: 'QR-Code',
    expiredText: 'QR-Code ist abgelaufen',
    refreshText: 'Aktualisieren',
    loadingText: 'Wird geladen...',
    errorText: 'QR-Code konnte nicht erstellt werden'
  },
  marquee: {
    ariaLabel: 'Laufender Inhalt'
  },
  image: {
    previewAriaLabel: 'Vorschau von {alt}',
    previewFallbackAlt: 'Bild',
    groupAriaLabel: 'Bildgruppe',
    loadErrorText: 'Bild konnte nicht geladen werden'
  },
  imageCompare: {
    ariaLabel: 'Bildvergleich',
    beforeAlt: 'Vorher',
    afterAlt: 'Nachher',
    valueText: '{percent}% der Vorher-Seite sind sichtbar'
  },
  text: {
    copyLabel: 'Kopieren',
    copiedLabel: 'Kopiert',
    copyFailedLabel: 'Kopieren fehlgeschlagen',
  },
  watermark: {
    imageErrorText: 'Wasserzeichenbild konnte nicht geladen werden'
  },
  segmented: {
    ariaLabel: 'Optionen'
  },
  descriptions: {
    colon: ':'
  },
  list: {
    avatarAlt: 'Profilbild',
    dragHandleAriaLabel: 'Neu anordnen'
  },
  masonry: {
    visualOrderText: 'Die sichtbare Reihenfolge weicht von der Quellreihenfolge ab'
  },
  scrollArea: {
    ariaLabel: 'Scrollbereich'
  },
  printLayout: {
    pageBreak: 'Seitenumbruch'
  },
  timeline: {
    pendingText: 'Wird geladen...',
    pendingReplacedText: 'Aktualisierung abgeschlossen'
  },
  workflowTimeline: {
    pending: 'Ausstehend',
    active: 'Aktiv',
    approved: 'Genehmigt',
    rejected: 'Abgelehnt',
    canceled: 'Abgebrochen',
    ariaLabel: 'Workflow-Zeitachse',
    actionsAriaLabel: 'Workflow-Aktionen',
    kindStart: 'Beginn',
    kindApprove: 'Freigabe',
    kindCc: 'Kopie',
    kindCondition: 'Bedingung',
    kindEnd: 'Ende',
    signSequential: 'Nacheinander',
    signCountersign: 'Gegenzeichnung',
    signOrsign: 'Oder-Zeichen',
    viewerAriaLabel: 'Arbeitsablauf',
    currentPath: 'Aktueller Pfad',
    rollbackPoint: 'Rücksprung',
    returnTarget: 'Zurück hierher',
    confirmApprove: 'Diesen Antrag freigeben?',
    confirmReject: 'Diesen Antrag ablehnen?',
    confirmCancel: 'Diesen Antrag zurückziehen?',
    confirmTransfer: 'Diesen Antrag weiterleiten?',
    confirmApproveDescription: '',
    confirmRejectDescription: 'Der Antragsteller wird benachrichtigt.',
    confirmCancelDescription: 'Nach dem Zurückziehen ist eine erneute Einreichung nötig.',
    confirmTransferDescription: 'Nach der Weiterleitung sind Sie nicht mehr der Freigeber.',
    commentPlaceholder: 'Freigabekommentar (optional)',
    commentRequired: 'Freigabekommentar erforderlich',
    offPath: 'Nicht genommener Zweig',
    ccNotified: 'Kopie gesendet',
    signCountersignHint: 'Gegenzeichnung (alle müssen zustimmen)',
    signOrsignHint: 'Oder-Zeichen (eine Zustimmung genügt)',
    signOrsignAny: 'Beliebige Person',
    signSequentialHint: 'Nacheinander (gemäß Liste)',
    legendAriaLabel: 'Pfadlegende',
    actorsProgress: '{approved}/{total} gezeichnet',
    actionApprove: 'Freigeben',
    actionReject: 'Ablehnen',
    actionTransfer: 'Weiterleiten',
    actionCancel: 'Zurückziehen',
    actionComment: 'Kommentar',
    actionAddsign: 'Freigeber hinzufügen',
    actionReturn: 'Zurückgeben',
    actionRequestChanges: 'Änderungen anfordern',
    addsignBefore: 'Davor',
    addsignAfter: 'Danach',
    addsignTag: 'Zusätzlicher Freigeber',
    confirmAddsign: 'Einen Freigeber hinzufügen?',
    confirmAddsignDescription: 'Die hinzugefügte Person gibt auf einem temporären Knoten frei.',
    confirmReturn: 'Diesen Antrag zurückgeben?',
    confirmReturnDescription: 'Die Instanz bleibt offen und startet am gewählten Knoten neu.',
    confirmRequestChanges: 'Änderungen anfordern?',
    confirmRequestChangesDescription:
      'Der Starter kann diese Instanz bearbeiten und erneut einreichen.',
    returnPickerTitle: 'Zurückgeben an',
    returnNoTargets: 'Kein Knoten zum Zurückgeben.',
    returnResumeResequence: 'Der Reihe nach erneut freigeben',
    returnResumeDirect: 'Am aktuellen Knoten fortsetzen',
    fieldEditable: 'Bearbeitbar',
    fieldReadonly: 'Schreibgeschützt',
    fieldHidden: 'Ausgeblendet',
    autoDecideManual: 'Manuell',
    autoDecideAutoPass: 'Automatisch freigeben',
    autoDecideAutoReject: 'Automatisch ablehnen',
    emptyApproverSkipPass: 'Überspringen und durchlassen',
    emptyApproverPause: 'Pausieren',
    emptyApproverTransferAdmin: 'An Admin weiterleiten',
    emptyApproverTransferUser: 'An Benutzer weiterleiten',
    moreActions: 'Mehr',
    taskBlocked: 'Wartet auf zusätzlichen Freigeber',
    commentRequiredBlock: 'Ein Kommentar ist erforderlich'
  },
  workflowDesigner: {
    ariaLabel: 'Workflow-Designer',
    addStep: 'Schritt hinzufügen',
    addChild: 'Unterschritt hinzufügen',
    removeStep: 'Entfernen',
    moveUp: 'Nach oben',
    moveDown: 'Nach unten',
    titleLabel: 'Titel',
    titlePlaceholder: 'Schritttitel',
    kindLabel: 'Art',
    signModeLabel: 'Freigabemodus',
    actorLabel: 'Bearbeiter',
    actorPlaceholder: 'Name des Bearbeiters',
    subpathEmpty: 'Kein Teilbaum auf diesem Pfad.',
    insertSibling: 'Dahinter einfügen',
    editPanelAriaLabel: 'Knoteneinstellungen',
    actorsLabel: 'Freigeber',
    addActor: 'Freigeber hinzufügen',
    removeActor: 'Entfernen',
    emptyHint: 'Zuerst Startknoten, dann darunter + Freigeber',
    tabApprovers: 'Freigeber',
    tabButtons: 'Aktionen',
    tabFieldPermissions: 'Formularberechtigungen',
    tabAdvanced: 'Erweitert',
    copyStep: 'Kopieren',
    paletteAriaLabel: 'Knoten einfügen',
    inspectorEmpty: 'Wählen Sie einen Knoten zum Bearbeiten',
    validationAriaLabel: 'Veröffentlichungsprüfung',
    validationMissingStart: 'Startknoten hinzufügen',
    validationMissingEnd: 'Endknoten hinzufügen',
    validationEmptyApprovers: 'Dieser Freigabeknoten hat keine Genehmiger',
    validationMissingBranches: 'Dieser Bedingungsknoten hat keine Zweige',
    validationButtonsAllDisabled: 'Alle Aktionsbuttons sind deaktiviert',
    sourceLabel: 'Genehmigerquelle',
    sourceFixed: 'Festgelegte Mitglieder',
    sourceSelf: 'Antragsteller',
    sourceStarterPick: 'Antragsteller wählt',
    sourceRole: 'Rolle',
    sourceGroup: 'Gruppe',
    sourceDeptLeader: 'Abteilungsleiter',
    sourceManagerChain: 'Vorgesetztenkette',
    sourceKeyLabel: 'Referenzschlüssel',
    sourceKeyPlaceholder: 'Schlüssel',
    sourceLevelLabel: 'Ebene',
    sourceUpToLabel: 'Stufen nach oben',
    sourceMultiple: 'Mehrfachauswahl erlauben',
    emptyApproverLabel: 'Wenn niemand ermittelt wird',
    actorIdPlaceholder: 'Mitglieder-ID',
    buttonEnabled: 'Aktiviert',
    buttonDisplayName: 'Anzeigename',
    buttonCommentRequired: 'Kommentar erforderlich',
    buttonPlacement: 'Platzierung',
    buttonPlacementBar: 'Leiste',
    buttonPlacementMore: 'Mehr',
    addsignPositions: 'Position für Zusatzgenehmiger',
    fieldPermissionsEmpty: 'Übergeben Sie ein Startschema, um Formularrechte zu bearbeiten',
    fieldPermissionAll: 'Alle',
    timeoutActionLabel: 'Bei Zeitüberschreitung',
    timeoutDurationLabel: 'Dauer der Zeitüberschreitung',
    timeoutRemind: 'Erinnern',
    autoDecideLabel: 'Automatische Entscheidung',
    addBranch: 'Zweig hinzufügen',
    removeBranch: 'Zweig entfernen',
    branchLabel: 'Zweigbezeichnung',
    branchExpression: 'Ausdruck',
    branchExpressionPlaceholder: 'amount > 1000',
    tabNotApplicable: 'Für diesen Knotentyp nicht verwendet',
    publishText: 'Veröffentlichen',
    publishBlocked: 'Veröffentlichung blockiert, bis dies behoben ist'
  },
  workflowDetailShell: {
    ariaLabel: 'Workflow-Details'
  },
  progress: {
    ariaLabel: 'Fortschritt'
  },
  splitter: {
    gutterAriaLabel: 'Bereiche {index} skalieren'
  },
  resizable: {
    handleAriaLabel: '{handle} skalieren'
  },
  upload: {
    dragAreaAriaLabel: 'Datei per Klick oder Ziehen hochladen',
    buttonAriaLabel: 'Datei hochladen',
    clickToUploadText: 'Zum Hochladen klicken',
    dragAndDropText: 'oder per Drag-and-drop',
    acceptInfoText: 'Erlaubt: {accept}',
    maxSizeInfoText: 'Max. Größe: {maxSize}',
    selectFileText: 'Datei auswählen',
    uploadedFilesAriaLabel: 'Hochgeladene Dateien',
    successAriaLabel: 'Erfolg',
    errorAriaLabel: 'Fehler',
    uploadingAriaLabel: 'Wird hochgeladen',
    removeFileAriaLabel: '{fileName} entfernen',
    previewFileAriaLabel: '{fileName} vorschau'
  },
  pagination: {
    totalText: 'Gesamt {total} Einträge',
    itemsPerPageText: '/ Seite',
    jumpToText: 'Gehe zu',
    pageText: 'Seite',
    prevPageAriaLabel: 'Vorherige Seite',
    nextPageAriaLabel: 'Nächste Seite',
    pageAriaLabel: 'Seite {page}',
    pageIndicatorText: 'Seite {current} von {total}',
    paginationAriaLabel: 'Paginierung',
    pageSizeAriaLabel: 'Elemente pro Seite'
  },
  table: {
    emptyText: 'Keine Daten',
    loadingText: 'Wird geladen',
    expandText: 'Erweitern',
    collapseText: 'Reduzieren',
    selectAllText: 'Alle auf dieser Seite auswählen',
    selectLoadedText: 'Alle geladenen Zeilen auswählen',
    tableAriaLabel: 'Datentabelle',
    selectRowAriaLabel: 'Zeile {row} auswählen',
    sortByText: 'Nach {column} sortieren',
    clearSortText: 'Sortierung löschen',
    toolbarAriaLabel: 'Datentabellen-Werkzeugleiste',
    searchPlaceholder: 'Suchen',
    searchButtonText: 'Suchen',
    selectedText: 'Ausgewählt',
    selectedItemsText: 'Einträge',
    columnSettingsText: 'Spalteneinstellungen',
    columnSettingsAriaLabel: 'Spalteneinstellungen',
    lockColumnAriaLabel: 'Spalte {column} sperren',
    unlockColumnAriaLabel: 'Spalte {column} entsperren',
    allText: 'Alle',
    filterPlaceholder: 'Filtern...',
    exportCsvText: 'CSV exportieren',
    exportCsvAriaLabel: 'Als CSV exportieren',
    selectionCountText: '{count} ausgewählt',
    sortAnnouncementText: 'Sortiert nach {column}, {direction}',
    sortAscendingText: 'aufsteigend',
    sortDescendingText: 'absteigend',
    sortNoneText: 'unsortiert',
    dragRowAriaLabel: 'Zeile {row} ziehen',
    expandRowAriaLabel: 'Zeile erweitern',
    collapseRowAriaLabel: 'Zeile reduzieren',
    sortMenuAriaLabel: 'Sortieren',
    filterColumnAriaLabel: '{column} filtern',
    groupHeaderText: '{key} ({count})'
  },
  dataExport: {
    triggerText: 'Exportieren',
    triggerAriaLabel: 'Daten exportieren',
    xlsxText: 'Excel exportieren',
    markdownText: 'Markdown exportieren',
    csvText: 'CSV exportieren',
    exportingText: 'Exportiere...',
    errorText: 'Export fehlgeschlagen'
  },
  timePicker: {
    hour: 'Stunde',
    minute: 'Minute',
    second: 'Sek',
    period: 'Tageszeit',
    now: 'Jetzt',
    ok: 'OK',
    start: 'Beginn',
    end: 'Ende',
    clear: 'Zeit löschen',
    toggle: 'Zeitauswahl öffnen',
    dialog: 'Zeitauswahl',
    selectTime: 'Zeit auswählen',
    selectTimeRange: 'Zeitbereich auswählen'
  },
  formWizard: {
    prevText: 'Zurück',
    nextText: 'Weiter',
    finishText: 'Fertigstellen',
    ariaLabel: 'Formularassistent',
    skippedText: 'Übersprungen'
  },
  schemaForm: {
    submitText: 'Absenden',
    resetText: 'Zurücksetzen',
    ariaLabel: 'Schema-Formular'
  },
  tour: {
    prevText: 'Zurück',
    nextText: 'Weiter',
    finishText: 'Fertigstellen',
    closeAriaLabel: 'Tour schließen',
    dialogAriaLabel: 'Rundgang',
    loadErrorText: 'Diese Tour konnte nicht geladen werden.',
  },
  calendar: {
    previousMonth: 'Vorheriger Monat',
    nextMonth: 'Nächster Monat',
    previousYear: 'Vorheriges Jahr',
    nextYear: 'Nächstes Jahr',
    eventCountText: '{n} Termine',
    switchToYear: 'Zur Jahresansicht wechseln',
    switchToMonth: 'Zur Monatsansicht wechseln'
  },
  fullscreen: {
    enterAriaLabel: 'Vollbild',
    exitAriaLabel: 'Vollbild beenden'
  },
  fileManager: {
    rootText: 'Stammverzeichnis',
    pathAriaLabel: 'Dateipfad',
    listboxAriaLabel: 'Dateien',
    searchAriaLabel: 'Dateien suchen',
    emptyText: 'Leerer Ordner',
    resultCountText: '{count} Elemente'
  },
  imageViewer: {
    dialogAriaLabel: 'Bildbetrachter',
    previewDialogAriaLabel: 'Bildvorschau',
    closeAriaLabel: 'Schließen',
    closePreviewAriaLabel: 'Vorschau schließen',
    previousImageAriaLabel: 'Vorheriges Bild',
    nextImageAriaLabel: 'Nächstes Bild',
    zoomOutAriaLabel: 'Verkleinern',
    resetAriaLabel: 'Zurücksetzen',
    zoomInAriaLabel: 'Vergrößern',
    rotateLeftAriaLabel: 'Nach links drehen',
    rotateRightAriaLabel: 'Nach rechts drehen',
    previewImageAriaLabel: 'Bild {index} von {total}'
  },
  imageEditor: {
    selectImageText: 'Bild auswählen',
    selectImageAriaLabel: 'Bild zum Zuschneiden und Hochladen auswählen',
    cropModalTitle: 'Bild zuschneiden',
    cropCancelText: 'Abbrechen',
    cropConfirmText: 'Zuschnitt bestätigen',
    cropperDialogAriaLabel: 'Bildzuschneider',
    imageToCropAriaLabel: 'Zuschneidendes Bild',
    moveCropAreaAriaLabel: 'Zuschnittbereich verschieben',
    resizeCropAreaAriaLabel: 'Zuschnittbereich {handle} skalieren',
    resizeHandleNw: 'oben links',
    resizeHandleN: 'oben',
    resizeHandleNe: 'oben rechts',
    resizeHandleE: 'rechts',
    resizeHandleSe: 'unten rechts',
    resizeHandleS: 'unten',
    resizeHandleSw: 'unten links',
    resizeHandleW: 'links',
    cropSizeText: '{width} mal {height}',
    loadingCropImageAriaLabel: 'Bild zum Zuschneiden wird geladen',
    loadErrorAriaLabel: 'Bild zum Zuschneiden konnte nicht geladen werden',
    fileTooLargeText: 'Die Datei überschreitet {maxSize}',
    fileTypeRejectedText: 'Dieser Dateityp wird nicht akzeptiert',
    annotationToolbarAriaLabel: 'Annotationstools',
    annotationEditorAriaLabel: 'Bildannotations-Editor',
    annotationCanvasAriaLabel: 'Annotationsleinwand',
    loadingAnnotationImageAriaLabel: 'Bild zum Annotieren wird geladen',
    loadAnnotationErrorAriaLabel: 'Bild zum Annotieren konnte nicht geladen werden',
    defaultAnnotationAlt: 'Bild zum Annotieren',
    annotationShapeAriaLabel: '{type}-Annotation',
    annotationLabeledShapeAriaLabel: '{label}, {type}-Annotation',
    selectToolText: 'Auswählen',
    rectangleToolText: 'Rechteck',
    ellipseToolText: 'Ellipsenform',
    polygonToolText: 'Vieleck',
    freehandToolText: 'Freihand',
    deleteText: 'Löschen',
    annotationLoadFailedText: 'Bild konnte nicht geladen werden.',
    annotationRetryText: 'Erneut versuchen',
    annotationTooSmallText: 'Markierung ist zu klein.',
    annotationPolygonIncompleteText: 'Ein Polygon braucht mindestens 3 Punkte.'
  },
  status: {
    tagCloseAriaLabel: '{label} schließen',
    badgeLabel: 'Benachrichtigung',
    badgeCountLabel: '{count} Benachrichtigungen'
  },
  taskBoard: {
    emptyColumnText: 'Keine Aufgaben',
    addCardText: 'Aufgabe hinzufügen',
    addColumnText: 'Spalte hinzufügen',
    newCardTitle: 'Neue Aufgabe',
    newColumnTitle: 'Neue Spalte',
    unassignedSwimlaneText: 'Nicht zugewiesen',
    wipLimitText: 'WIP-Limit: {limit}',
    dragHintText: 'Zum Verschieben ziehen',
    grabAnnouncementText: '{card}. Von {from} nach {to}. Position {position} von {count}.',
    boardAriaLabel: 'Aufgabenboard'
  },
  chatWindow: {
    emptyText: 'Keine Nachrichten',
    sendText: 'Senden',
    placeholder: 'Nachricht eingeben',
    sendingText: 'Senden...',
    sentText: 'Zugestellt',
    failedText: 'Senden fehlgeschlagen',
    messageListAriaLabel: 'Nachrichtenliste'
  },
  code: {
    copyLabel: 'Kopieren',
    copiedLabel: 'Kopiert',
    copyFailedLabel: 'Kopieren fehlgeschlagen',
    scrollLabel: 'Code'
  },
  commentThread: {
    emptyText: 'Keine Kommentare',
    replyPlaceholder: 'Antwort schreiben...',
    replyButtonText: 'Antworten',
    cancelReplyText: 'Abbrechen',
    likeText: 'Gefällt mir',
    likedText: 'Gefällt dir',
    replyText: 'Antworten',
    moreText: 'Mehr',
    loadMoreText: 'Mehr laden',
    collapseRepliesText: '▾ Antworten ausblenden',
    expandRepliesText: '▸ {count} Antworten anzeigen',
    listAriaLabel: 'Kommentarverlauf',
    replySubmitText: 'Antwort senden',
    remainingRepliesText: '{count} weitere anzeigen',
    duplicateIdText: 'Doppelte Kommentar-ID {id}',
    cycleText: 'Kommentarkreis bei {id}',
    maxDepthReachedText: 'Maximale Antworttiefe erreicht'
  },
  activityFeed: {
    emptyText: 'Keine Aktivität',
    loadingText: 'Wird geladen...',
    listAriaLabel: 'Aktivität',
    newItemText: 'Neue Aktivität: {title}',
    otherGroupTitle: 'Sonstiges'
  },
  notificationCenter: {
    title: 'Benachrichtigungen',
    emptyText: 'Keine Benachrichtigungen',
    loadingText: 'Wird geladen...',
    allLabel: 'Alle',
    unreadLabel: 'Ungelesen',
    readLabel: 'Gelesen',
    markAllReadText: 'Alle als gelesen markieren',
    markReadText: 'Als gelesen markieren',
    markUnreadText: 'Als ungelesen markieren',
    defaultGroupTitle: 'Standard',
    filterAriaLabel: 'Lesestatus',
    markedReadText: '{count} als gelesen markiert',
    newItemText: 'Neue Benachrichtigung: {title}',
    unreadCountText: '{count} ungelesen'
  },
  select: {
    doneText: 'Fertigstellen',
    placeholder: 'Option auswählen',
    emptyText: 'Keine Optionen gefunden',
    searchPlaceholder: 'Suchen',
    clearAriaLabel: 'Auswahl löschen',
    createOptionLabel: '"{label}" erstellen',
    moreCountText: '+{count} weitere',
    loadingText: 'Wird geladen...'
  },
  cascader: {
    levelLabel: 'Ebene {level}',
    backText: 'Zurück',
    expandAriaLabel: 'Erweitern',
    collapseAriaLabel: 'Einklappen'
  },
  treeSelect: {
    levelLabel: 'Ebene {level}',
    backText: 'Zurück',
    expandAriaLabel: 'Erweitern',
    collapseAriaLabel: 'Einklappen'
  },
  colorPicker: {
    trigger: 'Farbe wählen',
    panelTitle: 'Farbe',
    clear: 'Löschen',
    hue: 'Farbton',
    saturation: 'Sättigung',
    brightness: 'Helligkeit',
    alpha: 'Deckkraft',
    value: 'Farbwert',
    preview: 'Vorschau',
    selectPreset: '{color} auswählen',
    done: 'Fertig',
    formatHex: 'HEX',
    formatRgb: 'RGB',
    formatHsl: 'HSL',
    swatches: 'Farbfelder',
    primaryGroup: 'Primär',
    accentGroup: 'Akzent'
  },
  tabs: {
    addTabAriaLabel: 'Tab hinzufügen',
    closeTabAriaLabel: '{label} schließen',
    tablistAriaLabel: 'Registerkarten'
  },
  rate: {
    ariaLabel: 'Bewertung',
    valueText: '{value} Sterne'
  },
  avatarGroup: {
    ariaLabel: 'Avatargruppe',
    overflowAriaLabel: '{count} weitere'
  },
  carousel: {
    ariaLabel: 'Karussell',
    roleDescription: 'Karussell',
    slideRoleDescription: 'Folie',
    navigationAriaLabel: 'Karussellnavigation',
    previousSlideAriaLabel: 'Vorherige Folie',
    nextSlideAriaLabel: 'Nächste Folie',
    pauseAriaLabel: 'Automatische Wiedergabe pausieren',
    playAriaLabel: 'Automatische Wiedergabe starten',
    goToSlideAriaLabel: 'Zu Folie {index} springen',
    slideAriaLabel: 'Folie {index} von {total}'
  },
  transfer: {
    sourceTitle: 'Quelle',
    targetTitle: 'Ziel',
    searchAriaLabel: '{title} durchsuchen',
    itemsAriaLabel: '{title}-Elemente',
    moveToTargetAriaLabel: 'Auswahl ins Ziel verschieben',
    moveToSourceAriaLabel: 'Auswahl in die Quelle verschieben',
    selectAllAriaLabel: 'Alles in {title} auswählen'
  },
  chart: {
    legendAriaLabel: 'Diagrammlegende',
    pointAriaLabel: 'Punkt {index}: ({x}, {y})',
    seriesName: 'Reihe {index}',
    sliceName: 'Segment {index}',
    sliceAriaLabel: '{label}: {value} ({percent} %)',
    stageName: 'Stufe {index}',
    heatmapTooltip: '{x} × {y} = {value}',
    treemapTooltip: '{label}: {value} ({percent} %)',
    sunburstTooltip: '{label}: {value} ({percent} %)',
    orgChartAriaLabel: 'Organigramm',
    ganttAriaLabel: 'Gantt-Diagramm',
    ganttTaskAriaLabel: '{label}, {start} bis {end}{progress}',
    ganttTodayAriaLabel: 'Heute',
    ganttUnknownDate: 'unbekanntes Datum',
    chartAriaLabel: 'Diagramm',
    lineChartAriaLabel: 'Liniendiagramm',
    areaChartAriaLabel: 'Flächendiagramm',
    barChartAriaLabel: 'Balkendiagramm',
    scatterChartAriaLabel: 'Streudiagramm',
    pieChartAriaLabel: 'Kreisdiagramm',
    radarChartAriaLabel: 'Netzdiagramm',
    heatmapChartAriaLabel: 'Heatmap',
    heatmapSummary: '{rows} Zeilen und {columns} Spalten',
    funnelChartAriaLabel: 'Trichterdiagramm',
    sunburstChartAriaLabel: 'Sunburst-Diagramm',
    treemapChartAriaLabel: 'Treemap',
    gaugeChartAriaLabel: 'Messgerät'
  },
  codeEditor: {
    editorAriaLabel: 'Code-Editor'
  },

  markdownEditor: {
    formattingToolbarAriaLabel: 'Markdown-Formatierung',
    modeToolbarAriaLabel: 'Markdown-Ansichtsmodus',
    editorAriaLabel: 'Markdown-Editor',
    previewAriaLabel: 'Markdown-Vorschau',
    editModeLabel: 'Bearbeiten',
    splitModeLabel: 'Geteilt',
    previewModeLabel: 'Vorschau',
    showEditorText: 'Bearbeiten',
    showPreviewText: 'Vorschau',
    bold: 'Fett',
    italic: 'Kursiv',
    strikethrough: 'Durchgestrichen',
    heading: 'Überschrift',
    blockquote: 'Zitat',
    unorderedList: 'Aufzählung',
    orderedList: 'Nummerierte Liste',
    inlineCode: 'Inline-Code',
    codeBlock: 'Codeblock',
    link: 'Verknüpfung',
    image: 'Bild',
    table: 'Tabelle',
    horizontalRule: 'Trennlinie',
    placeholderStrong: 'fetter Text',
    placeholderEmphasis: 'Hervorhebung',
    placeholderDeleted: 'gelöschter Text',
    placeholderCode: 'Code',
    placeholderHeading: 'Überschrift',
    placeholderQuote: 'Zitat',
    placeholderListItem: 'Listeneintrag',
    placeholderLinkText: 'Linktext',
    placeholderImageAlt: 'Alternativtext',
    placeholderTable: '| Spalte | Wert |\n| --- | --- |\n| Name | Tigercat |'
  },
  richTextEditor: {
    formattingToolbarAriaLabel: 'Textformatierung',
    editorAriaLabel: 'Rich-Text-Editor',
    bold: 'Fett',
    italic: 'Kursiv',
    underline: 'Unterstrichen',
    strikethrough: 'Durchgestrichen',
    heading1: 'Überschrift 1',
    heading2: 'Überschrift 2',
    heading3: 'Überschrift 3',
    bulletList: 'Aufzählung',
    orderedList: 'Nummerierte Liste',
    blockquote: 'Zitat',
    codeBlock: 'Codeblock',
    link: 'Verknüpfung',
    image: 'Bild',
    horizontalRule: 'Linie',
    undo: 'Rückgängig',
    redo: 'Wiederholen',
    clear: 'Formatierung löschen',
    linkPrompt: 'URL eingeben',
    imagePrompt: 'Bild-URL eingeben'
  },
  cronEditor: {
    ariaLabel: 'Cron-Editor',
    expressionAriaLabel: 'Cron-Ausdruck',
    presetAriaLabel: 'Cron-Voreinstellung',
    presetPlaceholder: 'Voreinstellung',
    everyMinutePreset: 'Jede Minute',
    hourlyPreset: 'Stündlich',
    dailyPreset: 'Täglich',
    weeklyPreset: 'Wöchentlich',
    monthlyPreset: 'Monatlich',
    minuteLabel: 'Min.',
    hourLabel: 'Stunde',
    dayOfMonthLabel: 'Tag',
    monthLabel: 'Monat',
    dayOfWeekLabel: 'Wochentag',
    modeAnyLabel: 'Beliebig',
    modeEveryLabel: 'Jede',
    modeSpecificLabel: 'Bestimmte',
    modeRangeLabel: 'Bereich',
    modeCustomLabel: 'Benutzerdefiniert',
    modeAriaLabel: '{field}-Modus',
    stepAriaLabel: '{field}-Schritt',
    valueAriaLabel: '{field}-Wert',
    rangeStartAriaLabel: '{field}-Bereichsstart',
    rangeEndAriaLabel: '{field}-Bereichsende',
    customValueAriaLabel: '{field}-Benutzerwert',
    expressionFieldsError: 'Cron-Ausdruck muss 5 Felder enthalten',
    fieldRequiredError: '{field} ist erforderlich',
    invalidStepError: '{field} hat einen ungültigen Schrittautdruck',
    stepRangeError: '{field}-Schritt muss zwischen 1 und {max} liegen',
    fieldRangeError: '{field} muss zwischen {min} und {max} liegen',
    rangeOrderError: '{field}-Bereichsstart muss kleiner oder gleich dem Ende sein',
    invalidFieldError:
      '{field} muss *, eine Zahl, ein Bereich, ein Schritt oder eine Kommaliste sein'
  },
  formValidation: {
    required: 'Dieses Feld ist erforderlich',
    typeString: 'Wert muss eine Zeichenkette sein',
    typeNumber: 'Wert muss eine Zahl sein',
    typeBoolean: 'Wert muss ein Boolean sein',
    typeArray: 'Wert muss ein Array sein',
    typeObject: 'Wert muss ein Objekt sein',
    email: 'Bitte eine gültige E-Mail-Adresse eingeben',
    phone: 'Bitte eine gültige Telefonnummer eingeben',
    url: 'Bitte eine gültige URL eingeben',
    date: 'Bitte ein gültiges Datum eingeben',
    idCard: 'Bitte eine gültige Ausweisnummer eingeben',
    minLength: 'Mindestlänge ist {min} Zeichen',
    maxLength: 'Maximallänge ist {max} Zeichen',
    minValue: 'Mindestwert ist {min}',
    maxValue: 'Maximalwert ist {max}',
    minItems: 'Mindestens {min} Einträge erforderlich',
    maxItems: 'Höchstens {max} Einträge erlaubt',
    patternMismatch: 'Wert entspricht nicht dem erforderlichen Muster',
    validatorFailed: 'Validierung fehlgeschlagen',
    validatorError: 'Bei der Validierung ist ein Fehler aufgetreten'
  },
  inputOtp: {
    groupLabel: 'Einmalpasswort',
    slotLabel: 'Zeichen {index} von {total}'
  },
  tagsInput: {
    removeTagLabel: '{tag} entfernen',
    clearAllLabel: 'Alle Tags löschen',
    duplicateText: 'Bereits hinzugefügt',
    limitText: 'Tag-Limit erreicht'
  },
  input: {
    clearAriaLabel: 'Eingabe löschen',
    showPasswordAriaLabel: 'Passwort anzeigen',
    hidePasswordAriaLabel: 'Passwort verbergen'
  },
  inputNumber: {
    incrementAriaLabel: 'Erhöhen',
    decrementAriaLabel: 'Verringern',
    emptyAriaValueText: 'Leer'
  },
  slider: {
    ariaLabel: 'Schieberegler',
    minAriaLabel: 'Mindestwert',
    maxAriaLabel: 'Höchstwert'
  },
  signature: {
    ariaLabel: 'Unterschriftenfeld',
    undoText: 'Rückgängig',
    clearText: 'Löschen'
  },
  numberKeyboard: {
    ariaLabel: 'Ziffernblock',
    deleteText: 'Löschen',
    decimalAriaLabel: 'Dezimalzeichen',
    idCardXAriaLabel: 'Ausweis-X'
  },
  breadcrumb: {
    ariaLabel: 'Brotkrumen',
    expandAriaLabel: 'Eingeklappte Brotkrumen anzeigen'
  },
  pageHeader: {
    backAriaLabel: 'Zurück'
  },
  backTop: {
    ariaLabel: 'Nach oben'
  },
  anchor: {
    ariaLabel: 'Auf dieser Seite'
  },
  floatButton: {
    ariaLabel: 'Hinzufügen'
  },
  spotlight: {
    title: 'Befehlspalette',
    placeholder: 'Suchen'
  },
  scrollSpy: {
    ariaLabel: 'Abschnittsnavigation'
  },
  steps: {
    ariaLabel: 'Schritte',
    waitStatus: 'Ausstehend',
    processStatus: 'Aktuell',
    finishStatus: 'Abgeschlossen',
    errorStatus: 'Fehler'
  },
  tree: {
    ariaLabel: 'Baum',
    selectNode: '{label} auswählen',
    expand: 'Erweitern',
    collapse: 'Einklappen'
  }
}

export default deDE
