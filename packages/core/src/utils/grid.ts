...
function setSpanVars(vars: Record<string, string>, span: ColSpan): void {
  if (typeof span === 'number') {
    if (validateGridValue(span, 'span'))
      vars['--tiger-col-span'] = `${toPercentage(span)}%`;
    return;
  }
...
function setOffsetVars(vars: Record<string, string>, offset: ColOffset): void {
  if (typeof offset === 'number') {
    if (offset === 0) return;
    if (validateGridValue(offset, 'offset'))
      vars['--tiger-col-offset'] = `${toPercentage(offset)}%`;
    return;
  }
...
export function getColStyleVars(
  span?: ColSpan,
  offset?: ColOffset
