...
function createTailBuffer(maxLines = 120) {
  ...
  const getText = () => tailText(lines.join(os.EOL) + (pending ? os.EOL + pending : ''), maxLines)
...
function ensureExamplesDependenciesInstalled(exampleDirs) {
  ...
  console.log('')
  console.log('Example dependencies are missing. Installing workspace dependencies...')
  console.log('')
...
async function main() {
  ...
  if (needBuild) {
    ...
    if (status !== 0) {
      console.error('Error: Build failed. Please fix build errors and try again.')
      process.exit(status)
    }
  }
  ...
  if (installStatus !== 0) {
    console.error('Error: Failed to install dependencies required by examples.')
