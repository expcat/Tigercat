...
export const COMPONENT_PROP_PRIORITY = {
  ...
  Loading: ['text?', 'fullscreen?', 'spinning?', 'delay?', 'variant?', 'size?', 'color?'],
  Modal: [
    'closable?',
    'maskClosable?',
    'destroyOnClose?',
  ...
  Tour: ['open?', 'current?', 'closable?', 'maskClosable?', 'keyboard?', 'loadSteps?', 'locale?'],
...
export const REQUIRED_USAGE_SNIPPETS = {
  Vue: {
    ...
    Tour: '<Tour v-model:open="open" :steps="steps" />',
    ...
    Modal: '<Modal :open="open" title="Title">Body</Modal>',
    Drawer: '<Drawer :open="open" title="Title">Body</Drawer>',
  ...
  React: {
    ...
    Segmented: '<Segmented options={options} aria-label="View" />',
    Watermark: '<Watermark content="机密" />',
    Tour: '<Tour open={open} steps={steps} onOpenChange={setOpen} />',
