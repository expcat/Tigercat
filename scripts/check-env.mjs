  ...
  if (currentMajor >= requiredMajor) {
    console.log(
      `${c(
        'green',
        '✓'
      )} ${name}: ${current} (required: >= ${requiredMajor}.0.0)`
    );
    return true;
  }

  console.log(
    `${c('red', '✗')} ${name}: ${current} (required: >= ${requiredMajor}.0.0)`
  );
  return false;
}
...
function main() {
  ...
  if (!pnpmVersion) {
    ...
    console.log(
      `${c('yellow', 'ℹ')} Install pnpm: npm install -g pnpm@10.26.2`
    );
    hasErrors = true;
  ...
  if (existsSync('node_modules')) {
    ...
    try {
      ...
      if (
        !checkMajorVersion(
          'react',
          reactVersionInstalled,
          reactRequiredMajor || 0
        )
      ) {
    ...
    try {
      ...
      if (
        !checkMajorVersion('vue', vueVersionInstalled, vueRequiredMajor || 0)
      ) {
    ...
    console.log(
      `${c(
        'yellow',
        '⚠'
      )} Dependencies are not installed; skipping React/Vue version check`
    );
  }
  console.log('');
  ...
  if (!hasErrors) {
    console.log(
      `${c('green', '✓')} Environment check passed! You're ready to develop.`
    );
    console.log('');
