...
function relativeSpecifiers(content) {
  const specs = []
  const star = /export\s+\*\s+from\s+['"](\.[^'"]+)['"]/g
  const named = /export\s+(?:type\s+)?\{[^}]*\}\s+from\s+['"](\.[^'"]+)['"]/g
  let match
  while ((match = star.exec(content)) !== null) specs.push(match[1])
  while ((match = named.exec(content)) !== null) specs.push(match[1])
  return specs
}

function walkExportNames(entryFile) {
  const seen = new Set()
  const names = []
  ...
  function visit(file) {
    ...
    names.push(...extractExportNames(content))
    for (const specifier of relativeSpecifiers(content)) {
      visit(resolveTsSpecifier(file, specifier))
    }
  }
...
const coreExportNames = walkExportNames(join(CORE_SRC, 'index.ts'))
const coreSubpaths = {}
const { buildCorePackageEntries } = await import('./lib/public-components.mjs')
for (const entry of buildCorePackageEntries()) {
  if (!entry.source || entry.subpath === '.') continue
  coreSubpaths[entry.subpath] = walkExportNames(join(ROOT, 'packages/core', entry.source))
}
...
const snapshot = sortKeys({
  ...
  propsInterfaces,
  core: { exports: coreExportNames, subpaths: coreSubpaths },
  vue: {
